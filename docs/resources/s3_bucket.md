...
- `kms_master_key_id` (String) AWS Key Management Service (KMS) customer AWS KMS key ID to use for the default encryption. This parameter is allowed if and only if ``SSEAlgorithm`` is set to ``aws:kms`` or ``aws:kms:dsse``.
...
```shell
$ terraform import awscc_s3_bucket.example BucketName
```