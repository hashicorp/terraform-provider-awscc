...
- **ec2_inbound_permissions** (Attributes List, Max: 50) A range of IP addresses and port settings that allow inbound traffic to connect to server processes on an Amazon GameLift server. (see [below for nested schema](#nestedatt--ec2_inbound_permissions))
...
- **locations** (Attributes List, Min: 1, Max: 100) (see [below for nested schema](#nestedatt--locations))
...
- **server_processes** (Attributes List, Max: 50) A collection of server process configurations that describe which server processes to run on each instance in a fleet. (see [below for nested schema](#nestedatt--runtime_configuration--server_processes))
