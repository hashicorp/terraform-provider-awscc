...
- `outside_ip_address_type` (String) The type of IP address assigned to the outside interface of the customer gateway device.
 ...
 Default: ``PublicIpv4``
- `pre_shared_key_storage` (String) Specifies the storage mode for the pre-shared key (PSK). Valid values are ``Standard`` (stored in the S2Slong service) or ``SecretsManager`` (stored in AWS Secrets Manager).
