...
func (d durationType) ValueFromString(_ context.Context, s basetypes.StringValue) (basetypes.StringValuable, diag.Diagnostics) {
	var diags diag.Diagnostics
	...
	v, err := time.ParseDuration(s.ValueString())

	if err != nil {
		diags.AddError("parsing duration", err.Error())

		return nil, diags
	}

	return DurationValue(v), diags
}
...
func (d Duration) ToStringValue(context.Context) (basetypes.StringValue, diag.Diagnostics) {
	if d.IsUnknown() {
