// Copyright IBM Corp. 2021, 2026
// SPDX-License-Identifier: MPL-2.0

// Code generated by generators/singular-data-source/main.go; DO NOT EDIT.

package glue

import (
	"context"

	"github.com/hashicorp/terraform-plugin-framework/datasource"
	"github.com/hashicorp/terraform-plugin-framework/datasource/schema"
	"github.com/hashicorp/terraform-plugin-framework/types"
	"github.com/hashicorp/terraform-provider-awscc/internal/generic"
	"github.com/hashicorp/terraform-provider-awscc/internal/registry"
)

func init() {
	registry.AddDataSourceFactory("awscc_glue_connection_type", connectionTypeDataSource)
}

// connectionTypeDataSource returns the Terraform awscc_glue_connection_type data source.
// This Terraform data source corresponds to the CloudFormation AWS::Glue::ConnectionType resource.
func connectionTypeDataSource(ctx context.Context) (datasource.DataSource, error) {
	attributes := map[string]schema.Attribute{ /*START SCHEMA*/
		// Property: ConnectionProperties
		// CloudFormation resource type schema:
		//
		//	{
		//	  "additionalProperties": false,
		//	  "description": "Configuration that defines the base URL and additional request parameters needed during connection creation.",
		//	  "properties": {
		//	    "AdditionalRequestParameters": {
		//	      "description": "Key-value pairs of additional request parameters.",
		//	      "items": {
		//	        "additionalProperties": false,
		//	        "description": "Defines a property configuration for connection types.",
		//	        "properties": {
		//	          "AllowedValues": {
		//	            "description": "A list of allowed values for the property.",
		//	            "items": {
		//	              "type": "string"
		//	            },
		//	            "type": "array"
		//	          },
		//	          "DefaultValue": {
		//	            "description": "The default value for the property.",
		//	            "type": "string"
		//	          },
		//	          "KeyOverride": {
		//	            "description": "A key name to use when sending this property in API requests.",
		//	            "maxLength": 128,
		//	            "minLength": 1,
		//	            "pattern": "^[a-zA-Z0-9_-]+$",
		//	            "type": "string"
		//	          },
		//	          "Name": {
		//	            "description": "The name of the property.",
		//	            "maxLength": 128,
		//	            "minLength": 1,
		//	            "type": "string"
		//	          },
		//	          "PropertyLocation": {
		//	            "description": "Specifies where this property should be included in REST requests.",
		//	            "enum": [
		//	              "HEADER",
		//	              "BODY",
		//	              "QUERY_PARAM",
		//	              "PATH"
		//	            ],
		//	            "type": "string"
		//	          },
		//	          "PropertyType": {
		//	            "description": "The data type of this property.",
		//	            "enum": [
		//	              "USER_INPUT",
		//	              "SECRET",
		//	              "READ_ONLY",
		//	              "UNUSED",
		//	              "SECRET_OR_USER_INPUT"
		//	            ],
		//	            "type": "string"
		//	          },
		//	          "Required": {
		//	            "description": "Indicates whether the property is required.",
		//	            "type": "boolean"
		//	          }
		//	        },
		//	        "required": [
		//	          "Name",
		//	          "Required",
		//	          "PropertyType"
		//	        ],
		//	        "type": "object"
		//	      },
		//	      "type": "array"
		//	    },
		//	    "Url": {
		//	      "additionalProperties": false,
		//	      "description": "Defines a property configuration for connection types.",
		//	      "properties": {
		//	        "AllowedValues": {
		//	          "description": "A list of allowed values for the property.",
		//	          "items": {
		//	            "type": "string"
		//	          },
		//	          "type": "array"
		//	        },
		//	        "DefaultValue": {
		//	          "description": "The default value for the property.",
		//	          "type": "string"
		//	        },
		//	        "KeyOverride": {
		//	          "description": "A key name to use when sending this property in API requests.",
		//	          "maxLength": 128,
		//	          "minLength": 1,
		//	          "pattern": "^[a-zA-Z0-9_-]+$",
		//	          "type": "string"
		//	        },
		//	        "Name": {
		//	          "description": "The name of the property.",
		//	          "maxLength": 128,
		//	          "minLength": 1,
		//	          "type": "string"
		//	        },
		//	        "PropertyLocation": {
		//	          "description": "Specifies where this property should be included in REST requests.",
		//	          "enum": [
		//	            "HEADER",
		//	            "BODY",
		//	            "QUERY_PARAM",
		//	            "PATH"
		//	          ],
		//	          "type": "string"
		//	        },
		//	        "PropertyType": {
		//	          "description": "The data type of this property.",
		//	          "enum": [
		//	            "USER_INPUT",
		//	            "SECRET",
		//	            "READ_ONLY",
		//	            "UNUSED",
		//	            "SECRET_OR_USER_INPUT"
		//	          ],
		//	          "type": "string"
		//	        },
		//	        "Required": {
		//	          "description": "Indicates whether the property is required.",
		//	          "type": "boolean"
		//	        }
		//	      },
		//	      "required": [
		//	        "Name",
		//	        "Required",
		//	        "PropertyType"
		//	      ],
		//	      "type": "object"
		//	    }
		//	  },
		//	  "type": "object"
		//	}
		"connection_properties": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
			Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
				// Property: AdditionalRequestParameters
				"additional_request_parameters": schema.ListNestedAttribute{ /*START ATTRIBUTE*/
					NestedObject: schema.NestedAttributeObject{ /*START NESTED OBJECT*/
						Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
							// Property: AllowedValues
							"allowed_values": schema.ListAttribute{ /*START ATTRIBUTE*/
								ElementType: types.StringType,
								Description: "A list of allowed values for the property.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: DefaultValue
							"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
								Description: "The default value for the property.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: KeyOverride
							"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
								Description: "A key name to use when sending this property in API requests.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: Name
							"name": schema.StringAttribute{ /*START ATTRIBUTE*/
								Description: "The name of the property.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: PropertyLocation
							"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
								Description: "Specifies where this property should be included in REST requests.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: PropertyType
							"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
								Description: "The data type of this property.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: Required
							"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
								Description: "Indicates whether the property is required.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
						}, /*END SCHEMA*/
					}, /*END NESTED OBJECT*/
					Description: "Key-value pairs of additional request parameters.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: Url
				"url": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
					Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
						// Property: AllowedValues
						"allowed_values": schema.ListAttribute{ /*START ATTRIBUTE*/
							ElementType: types.StringType,
							Description: "A list of allowed values for the property.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
						// Property: DefaultValue
						"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
							Description: "The default value for the property.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
						// Property: KeyOverride
						"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
							Description: "A key name to use when sending this property in API requests.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
						// Property: Name
						"name": schema.StringAttribute{ /*START ATTRIBUTE*/
							Description: "The name of the property.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
						// Property: PropertyLocation
						"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
							Description: "Specifies where this property should be included in REST requests.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
						// Property: PropertyType
						"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
							Description: "The data type of this property.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
						// Property: Required
						"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
							Description: "Indicates whether the property is required.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
					}, /*END SCHEMA*/
					Description: "Defines a property configuration for connection types.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
			}, /*END SCHEMA*/
			Description: "Configuration that defines the base URL and additional request parameters needed during connection creation.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: ConnectionType
		// CloudFormation resource type schema:
		//
		//	{
		//	  "description": "The name of the connection type. Must be prefixed with REST-.",
		//	  "maxLength": 255,
		//	  "minLength": 1,
		//	  "type": "string"
		//	}
		"connection_type": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "The name of the connection type. Must be prefixed with REST-.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: ConnectionTypeArn
		// CloudFormation resource type schema:
		//
		//	{
		//	  "description": "The Amazon Resource Name (ARN) of the registered connection type.",
		//	  "maxLength": 10240,
		//	  "minLength": 1,
		//	  "pattern": "^arn:aws(-(cn|us-gov|iso(-[bef])?))?:glue:",
		//	  "type": "string"
		//	}
		"connection_type_arn": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "The Amazon Resource Name (ARN) of the registered connection type.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: ConnectorAuthenticationConfiguration
		// CloudFormation resource type schema:
		//
		//	{
		//	  "additionalProperties": false,
		//	  "description": "Configuration that defines supported authentication types and required properties.",
		//	  "properties": {
		//	    "AuthenticationTypes": {
		//	      "description": "A list of authentication types supported.",
		//	      "items": {
		//	        "enum": [
		//	          "BASIC",
		//	          "OAUTH2",
		//	          "CUSTOM",
		//	          "IAM"
		//	        ],
		//	        "type": "string"
		//	      },
		//	      "type": "array"
		//	    },
		//	    "BasicAuthenticationProperties": {
		//	      "additionalProperties": false,
		//	      "description": "Basic authentication configuration.",
		//	      "properties": {
		//	        "Password": {
		//	          "additionalProperties": false,
		//	          "description": "Defines a secret property configuration. SECRET-type properties cannot have DefaultValue or AllowedValues.",
		//	          "properties": {
		//	            "KeyOverride": {
		//	              "description": "A key name to use when sending this property in API requests.",
		//	              "maxLength": 128,
		//	              "minLength": 1,
		//	              "pattern": "^[a-zA-Z0-9_-]+$",
		//	              "type": "string"
		//	            },
		//	            "Name": {
		//	              "description": "The name of the property.",
		//	              "maxLength": 128,
		//	              "minLength": 1,
		//	              "type": "string"
		//	            },
		//	            "PropertyLocation": {
		//	              "description": "Specifies where this property should be included in REST requests.",
		//	              "enum": [
		//	                "HEADER",
		//	                "BODY",
		//	                "QUERY_PARAM",
		//	                "PATH"
		//	              ],
		//	              "type": "string"
		//	            },
		//	            "PropertyType": {
		//	              "description": "The data type of this property. Must be SECRET for secret properties.",
		//	              "enum": [
		//	                "SECRET"
		//	              ],
		//	              "type": "string"
		//	            },
		//	            "Required": {
		//	              "description": "Indicates whether the property is required.",
		//	              "type": "boolean"
		//	            }
		//	          },
		//	          "required": [
		//	            "Name",
		//	            "Required",
		//	            "PropertyType"
		//	          ],
		//	          "type": "object"
		//	        },
		//	        "Username": {
		//	          "additionalProperties": false,
		//	          "description": "Defines a secret property configuration. SECRET-type properties cannot have DefaultValue or AllowedValues.",
		//	          "properties": {
		//	            "KeyOverride": {
		//	              "description": "A key name to use when sending this property in API requests.",
		//	              "maxLength": 128,
		//	              "minLength": 1,
		//	              "pattern": "^[a-zA-Z0-9_-]+$",
		//	              "type": "string"
		//	            },
		//	            "Name": {
		//	              "description": "The name of the property.",
		//	              "maxLength": 128,
		//	              "minLength": 1,
		//	              "type": "string"
		//	            },
		//	            "PropertyLocation": {
		//	              "description": "Specifies where this property should be included in REST requests.",
		//	              "enum": [
		//	                "HEADER",
		//	                "BODY",
		//	                "QUERY_PARAM",
		//	                "PATH"
		//	              ],
		//	              "type": "string"
		//	            },
		//	            "PropertyType": {
		//	              "description": "The data type of this property. Must be SECRET for secret properties.",
		//	              "enum": [
		//	                "SECRET"
		//	              ],
		//	              "type": "string"
		//	            },
		//	            "Required": {
		//	              "description": "Indicates whether the property is required.",
		//	              "type": "boolean"
		//	            }
		//	          },
		//	          "required": [
		//	            "Name",
		//	            "Required",
		//	            "PropertyType"
		//	          ],
		//	          "type": "object"
		//	        }
		//	      },
		//	      "type": "object"
		//	    },
		//	    "CustomAuthenticationProperties": {
		//	      "additionalProperties": false,
		//	      "description": "Custom authentication configuration.",
		//	      "properties": {
		//	        "AuthenticationParameters": {
		//	          "items": {
		//	            "additionalProperties": false,
		//	            "description": "Defines a secret property configuration. SECRET-type properties cannot have DefaultValue or AllowedValues.",
		//	            "properties": {
		//	              "KeyOverride": {
		//	                "description": "A key name to use when sending this property in API requests.",
		//	                "maxLength": 128,
		//	                "minLength": 1,
		//	                "pattern": "^[a-zA-Z0-9_-]+$",
		//	                "type": "string"
		//	              },
		//	              "Name": {
		//	                "description": "The name of the property.",
		//	                "maxLength": 128,
		//	                "minLength": 1,
		//	                "type": "string"
		//	              },
		//	              "PropertyLocation": {
		//	                "description": "Specifies where this property should be included in REST requests.",
		//	                "enum": [
		//	                  "HEADER",
		//	                  "BODY",
		//	                  "QUERY_PARAM",
		//	                  "PATH"
		//	                ],
		//	                "type": "string"
		//	              },
		//	              "PropertyType": {
		//	                "description": "The data type of this property. Must be SECRET for secret properties.",
		//	                "enum": [
		//	                  "SECRET"
		//	                ],
		//	                "type": "string"
		//	              },
		//	              "Required": {
		//	                "description": "Indicates whether the property is required.",
		//	                "type": "boolean"
		//	              }
		//	            },
		//	            "required": [
		//	              "Name",
		//	              "Required",
		//	              "PropertyType"
		//	            ],
		//	            "type": "object"
		//	          },
		//	          "type": "array"
		//	        }
		//	      },
		//	      "required": [
		//	        "AuthenticationParameters"
		//	      ],
		//	      "type": "object"
		//	    },
		//	    "OAuth2Properties": {
		//	      "additionalProperties": false,
		//	      "description": "OAuth2 configuration container.",
		//	      "properties": {
		//	        "AuthorizationCodeProperties": {
		//	          "additionalProperties": false,
		//	          "description": "OAuth2 authorization code configuration.",
		//	          "properties": {
		//	            "AuthorizationCode": {
		//	              "additionalProperties": false,
		//	              "description": "Defines a property configuration for connection types.",
		//	              "properties": {
		//	                "AllowedValues": {
		//	                  "description": "A list of allowed values for the property.",
		//	                  "items": {
		//	                    "type": "string"
		//	                  },
		//	                  "type": "array"
		//	                },
		//	                "DefaultValue": {
		//	                  "description": "The default value for the property.",
		//	                  "type": "string"
		//	                },
		//	                "KeyOverride": {
		//	                  "description": "A key name to use when sending this property in API requests.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "pattern": "^[a-zA-Z0-9_-]+$",
		//	                  "type": "string"
		//	                },
		//	                "Name": {
		//	                  "description": "The name of the property.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "type": "string"
		//	                },
		//	                "PropertyLocation": {
		//	                  "description": "Specifies where this property should be included in REST requests.",
		//	                  "enum": [
		//	                    "HEADER",
		//	                    "BODY",
		//	                    "QUERY_PARAM",
		//	                    "PATH"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "PropertyType": {
		//	                  "description": "The data type of this property.",
		//	                  "enum": [
		//	                    "USER_INPUT",
		//	                    "SECRET",
		//	                    "READ_ONLY",
		//	                    "UNUSED",
		//	                    "SECRET_OR_USER_INPUT"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "Required": {
		//	                  "description": "Indicates whether the property is required.",
		//	                  "type": "boolean"
		//	                }
		//	              },
		//	              "required": [
		//	                "Name",
		//	                "Required",
		//	                "PropertyType"
		//	              ],
		//	              "type": "object"
		//	            },
		//	            "AuthorizationCodeUrl": {
		//	              "additionalProperties": false,
		//	              "description": "Defines a property configuration for connection types.",
		//	              "properties": {
		//	                "AllowedValues": {
		//	                  "description": "A list of allowed values for the property.",
		//	                  "items": {
		//	                    "type": "string"
		//	                  },
		//	                  "type": "array"
		//	                },
		//	                "DefaultValue": {
		//	                  "description": "The default value for the property.",
		//	                  "type": "string"
		//	                },
		//	                "KeyOverride": {
		//	                  "description": "A key name to use when sending this property in API requests.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "pattern": "^[a-zA-Z0-9_-]+$",
		//	                  "type": "string"
		//	                },
		//	                "Name": {
		//	                  "description": "The name of the property.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "type": "string"
		//	                },
		//	                "PropertyLocation": {
		//	                  "description": "Specifies where this property should be included in REST requests.",
		//	                  "enum": [
		//	                    "HEADER",
		//	                    "BODY",
		//	                    "QUERY_PARAM",
		//	                    "PATH"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "PropertyType": {
		//	                  "description": "The data type of this property.",
		//	                  "enum": [
		//	                    "USER_INPUT",
		//	                    "SECRET",
		//	                    "READ_ONLY",
		//	                    "UNUSED",
		//	                    "SECRET_OR_USER_INPUT"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "Required": {
		//	                  "description": "Indicates whether the property is required.",
		//	                  "type": "boolean"
		//	                }
		//	              },
		//	              "required": [
		//	                "Name",
		//	                "Required",
		//	                "PropertyType"
		//	              ],
		//	              "type": "object"
		//	            },
		//	            "ClientId": {
		//	              "additionalProperties": false,
		//	              "description": "Defines a secret property configuration. SECRET-type properties cannot have DefaultValue or AllowedValues.",
		//	              "properties": {
		//	                "KeyOverride": {
		//	                  "description": "A key name to use when sending this property in API requests.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "pattern": "^[a-zA-Z0-9_-]+$",
		//	                  "type": "string"
		//	                },
		//	                "Name": {
		//	                  "description": "The name of the property.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "type": "string"
		//	                },
		//	                "PropertyLocation": {
		//	                  "description": "Specifies where this property should be included in REST requests.",
		//	                  "enum": [
		//	                    "HEADER",
		//	                    "BODY",
		//	                    "QUERY_PARAM",
		//	                    "PATH"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "PropertyType": {
		//	                  "description": "The data type of this property. Must be SECRET for secret properties.",
		//	                  "enum": [
		//	                    "SECRET"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "Required": {
		//	                  "description": "Indicates whether the property is required.",
		//	                  "type": "boolean"
		//	                }
		//	              },
		//	              "required": [
		//	                "Name",
		//	                "Required",
		//	                "PropertyType"
		//	              ],
		//	              "type": "object"
		//	            },
		//	            "ClientSecret": {
		//	              "additionalProperties": false,
		//	              "description": "Defines a secret property configuration. SECRET-type properties cannot have DefaultValue or AllowedValues.",
		//	              "properties": {
		//	                "KeyOverride": {
		//	                  "description": "A key name to use when sending this property in API requests.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "pattern": "^[a-zA-Z0-9_-]+$",
		//	                  "type": "string"
		//	                },
		//	                "Name": {
		//	                  "description": "The name of the property.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "type": "string"
		//	                },
		//	                "PropertyLocation": {
		//	                  "description": "Specifies where this property should be included in REST requests.",
		//	                  "enum": [
		//	                    "HEADER",
		//	                    "BODY",
		//	                    "QUERY_PARAM",
		//	                    "PATH"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "PropertyType": {
		//	                  "description": "The data type of this property. Must be SECRET for secret properties.",
		//	                  "enum": [
		//	                    "SECRET"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "Required": {
		//	                  "description": "Indicates whether the property is required.",
		//	                  "type": "boolean"
		//	                }
		//	              },
		//	              "required": [
		//	                "Name",
		//	                "Required",
		//	                "PropertyType"
		//	              ],
		//	              "type": "object"
		//	            },
		//	            "ContentType": {
		//	              "enum": [
		//	                "APPLICATION_JSON",
		//	                "URL_ENCODED"
		//	              ],
		//	              "type": "string"
		//	            },
		//	            "Prompt": {
		//	              "additionalProperties": false,
		//	              "description": "Defines a property configuration for connection types.",
		//	              "properties": {
		//	                "AllowedValues": {
		//	                  "description": "A list of allowed values for the property.",
		//	                  "items": {
		//	                    "type": "string"
		//	                  },
		//	                  "type": "array"
		//	                },
		//	                "DefaultValue": {
		//	                  "description": "The default value for the property.",
		//	                  "type": "string"
		//	                },
		//	                "KeyOverride": {
		//	                  "description": "A key name to use when sending this property in API requests.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "pattern": "^[a-zA-Z0-9_-]+$",
		//	                  "type": "string"
		//	                },
		//	                "Name": {
		//	                  "description": "The name of the property.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "type": "string"
		//	                },
		//	                "PropertyLocation": {
		//	                  "description": "Specifies where this property should be included in REST requests.",
		//	                  "enum": [
		//	                    "HEADER",
		//	                    "BODY",
		//	                    "QUERY_PARAM",
		//	                    "PATH"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "PropertyType": {
		//	                  "description": "The data type of this property.",
		//	                  "enum": [
		//	                    "USER_INPUT",
		//	                    "SECRET",
		//	                    "READ_ONLY",
		//	                    "UNUSED",
		//	                    "SECRET_OR_USER_INPUT"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "Required": {
		//	                  "description": "Indicates whether the property is required.",
		//	                  "type": "boolean"
		//	                }
		//	              },
		//	              "required": [
		//	                "Name",
		//	                "Required",
		//	                "PropertyType"
		//	              ],
		//	              "type": "object"
		//	            },
		//	            "RedirectUri": {
		//	              "additionalProperties": false,
		//	              "description": "Defines a property configuration for connection types.",
		//	              "properties": {
		//	                "AllowedValues": {
		//	                  "description": "A list of allowed values for the property.",
		//	                  "items": {
		//	                    "type": "string"
		//	                  },
		//	                  "type": "array"
		//	                },
		//	                "DefaultValue": {
		//	                  "description": "The default value for the property.",
		//	                  "type": "string"
		//	                },
		//	                "KeyOverride": {
		//	                  "description": "A key name to use when sending this property in API requests.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "pattern": "^[a-zA-Z0-9_-]+$",
		//	                  "type": "string"
		//	                },
		//	                "Name": {
		//	                  "description": "The name of the property.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "type": "string"
		//	                },
		//	                "PropertyLocation": {
		//	                  "description": "Specifies where this property should be included in REST requests.",
		//	                  "enum": [
		//	                    "HEADER",
		//	                    "BODY",
		//	                    "QUERY_PARAM",
		//	                    "PATH"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "PropertyType": {
		//	                  "description": "The data type of this property.",
		//	                  "enum": [
		//	                    "USER_INPUT",
		//	                    "SECRET",
		//	                    "READ_ONLY",
		//	                    "UNUSED",
		//	                    "SECRET_OR_USER_INPUT"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "Required": {
		//	                  "description": "Indicates whether the property is required.",
		//	                  "type": "boolean"
		//	                }
		//	              },
		//	              "required": [
		//	                "Name",
		//	                "Required",
		//	                "PropertyType"
		//	              ],
		//	              "type": "object"
		//	            },
		//	            "RequestMethod": {
		//	              "enum": [
		//	                "GET",
		//	                "POST"
		//	              ],
		//	              "type": "string"
		//	            },
		//	            "Scope": {
		//	              "additionalProperties": false,
		//	              "description": "Defines a property configuration for connection types.",
		//	              "properties": {
		//	                "AllowedValues": {
		//	                  "description": "A list of allowed values for the property.",
		//	                  "items": {
		//	                    "type": "string"
		//	                  },
		//	                  "type": "array"
		//	                },
		//	                "DefaultValue": {
		//	                  "description": "The default value for the property.",
		//	                  "type": "string"
		//	                },
		//	                "KeyOverride": {
		//	                  "description": "A key name to use when sending this property in API requests.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "pattern": "^[a-zA-Z0-9_-]+$",
		//	                  "type": "string"
		//	                },
		//	                "Name": {
		//	                  "description": "The name of the property.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "type": "string"
		//	                },
		//	                "PropertyLocation": {
		//	                  "description": "Specifies where this property should be included in REST requests.",
		//	                  "enum": [
		//	                    "HEADER",
		//	                    "BODY",
		//	                    "QUERY_PARAM",
		//	                    "PATH"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "PropertyType": {
		//	                  "description": "The data type of this property.",
		//	                  "enum": [
		//	                    "USER_INPUT",
		//	                    "SECRET",
		//	                    "READ_ONLY",
		//	                    "UNUSED",
		//	                    "SECRET_OR_USER_INPUT"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "Required": {
		//	                  "description": "Indicates whether the property is required.",
		//	                  "type": "boolean"
		//	                }
		//	              },
		//	              "required": [
		//	                "Name",
		//	                "Required",
		//	                "PropertyType"
		//	              ],
		//	              "type": "object"
		//	            },
		//	            "TokenUrl": {
		//	              "additionalProperties": false,
		//	              "description": "Defines a property configuration for connection types.",
		//	              "properties": {
		//	                "AllowedValues": {
		//	                  "description": "A list of allowed values for the property.",
		//	                  "items": {
		//	                    "type": "string"
		//	                  },
		//	                  "type": "array"
		//	                },
		//	                "DefaultValue": {
		//	                  "description": "The default value for the property.",
		//	                  "type": "string"
		//	                },
		//	                "KeyOverride": {
		//	                  "description": "A key name to use when sending this property in API requests.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "pattern": "^[a-zA-Z0-9_-]+$",
		//	                  "type": "string"
		//	                },
		//	                "Name": {
		//	                  "description": "The name of the property.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "type": "string"
		//	                },
		//	                "PropertyLocation": {
		//	                  "description": "Specifies where this property should be included in REST requests.",
		//	                  "enum": [
		//	                    "HEADER",
		//	                    "BODY",
		//	                    "QUERY_PARAM",
		//	                    "PATH"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "PropertyType": {
		//	                  "description": "The data type of this property.",
		//	                  "enum": [
		//	                    "USER_INPUT",
		//	                    "SECRET",
		//	                    "READ_ONLY",
		//	                    "UNUSED",
		//	                    "SECRET_OR_USER_INPUT"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "Required": {
		//	                  "description": "Indicates whether the property is required.",
		//	                  "type": "boolean"
		//	                }
		//	              },
		//	              "required": [
		//	                "Name",
		//	                "Required",
		//	                "PropertyType"
		//	              ],
		//	              "type": "object"
		//	            },
		//	            "TokenUrlParameters": {
		//	              "items": {
		//	                "additionalProperties": false,
		//	                "description": "Defines a property configuration for connection types.",
		//	                "properties": {
		//	                  "AllowedValues": {
		//	                    "description": "A list of allowed values for the property.",
		//	                    "items": {
		//	                      "type": "string"
		//	                    },
		//	                    "type": "array"
		//	                  },
		//	                  "DefaultValue": {
		//	                    "description": "The default value for the property.",
		//	                    "type": "string"
		//	                  },
		//	                  "KeyOverride": {
		//	                    "description": "A key name to use when sending this property in API requests.",
		//	                    "maxLength": 128,
		//	                    "minLength": 1,
		//	                    "pattern": "^[a-zA-Z0-9_-]+$",
		//	                    "type": "string"
		//	                  },
		//	                  "Name": {
		//	                    "description": "The name of the property.",
		//	                    "maxLength": 128,
		//	                    "minLength": 1,
		//	                    "type": "string"
		//	                  },
		//	                  "PropertyLocation": {
		//	                    "description": "Specifies where this property should be included in REST requests.",
		//	                    "enum": [
		//	                      "HEADER",
		//	                      "BODY",
		//	                      "QUERY_PARAM",
		//	                      "PATH"
		//	                    ],
		//	                    "type": "string"
		//	                  },
		//	                  "PropertyType": {
		//	                    "description": "The data type of this property.",
		//	                    "enum": [
		//	                      "USER_INPUT",
		//	                      "SECRET",
		//	                      "READ_ONLY",
		//	                      "UNUSED",
		//	                      "SECRET_OR_USER_INPUT"
		//	                    ],
		//	                    "type": "string"
		//	                  },
		//	                  "Required": {
		//	                    "description": "Indicates whether the property is required.",
		//	                    "type": "boolean"
		//	                  }
		//	                },
		//	                "required": [
		//	                  "Name",
		//	                  "Required",
		//	                  "PropertyType"
		//	                ],
		//	                "type": "object"
		//	              },
		//	              "type": "array"
		//	            }
		//	          },
		//	          "type": "object"
		//	        },
		//	        "ClientCredentialsProperties": {
		//	          "additionalProperties": false,
		//	          "description": "OAuth2 client credentials configuration.",
		//	          "properties": {
		//	            "ClientId": {
		//	              "additionalProperties": false,
		//	              "description": "Defines a secret property configuration. SECRET-type properties cannot have DefaultValue or AllowedValues.",
		//	              "properties": {
		//	                "KeyOverride": {
		//	                  "description": "A key name to use when sending this property in API requests.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "pattern": "^[a-zA-Z0-9_-]+$",
		//	                  "type": "string"
		//	                },
		//	                "Name": {
		//	                  "description": "The name of the property.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "type": "string"
		//	                },
		//	                "PropertyLocation": {
		//	                  "description": "Specifies where this property should be included in REST requests.",
		//	                  "enum": [
		//	                    "HEADER",
		//	                    "BODY",
		//	                    "QUERY_PARAM",
		//	                    "PATH"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "PropertyType": {
		//	                  "description": "The data type of this property. Must be SECRET for secret properties.",
		//	                  "enum": [
		//	                    "SECRET"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "Required": {
		//	                  "description": "Indicates whether the property is required.",
		//	                  "type": "boolean"
		//	                }
		//	              },
		//	              "required": [
		//	                "Name",
		//	                "Required",
		//	                "PropertyType"
		//	              ],
		//	              "type": "object"
		//	            },
		//	            "ClientSecret": {
		//	              "additionalProperties": false,
		//	              "description": "Defines a secret property configuration. SECRET-type properties cannot have DefaultValue or AllowedValues.",
		//	              "properties": {
		//	                "KeyOverride": {
		//	                  "description": "A key name to use when sending this property in API requests.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "pattern": "^[a-zA-Z0-9_-]+$",
		//	                  "type": "string"
		//	                },
		//	                "Name": {
		//	                  "description": "The name of the property.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "type": "string"
		//	                },
		//	                "PropertyLocation": {
		//	                  "description": "Specifies where this property should be included in REST requests.",
		//	                  "enum": [
		//	                    "HEADER",
		//	                    "BODY",
		//	                    "QUERY_PARAM",
		//	                    "PATH"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "PropertyType": {
		//	                  "description": "The data type of this property. Must be SECRET for secret properties.",
		//	                  "enum": [
		//	                    "SECRET"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "Required": {
		//	                  "description": "Indicates whether the property is required.",
		//	                  "type": "boolean"
		//	                }
		//	              },
		//	              "required": [
		//	                "Name",
		//	                "Required",
		//	                "PropertyType"
		//	              ],
		//	              "type": "object"
		//	            },
		//	            "ContentType": {
		//	              "enum": [
		//	                "APPLICATION_JSON",
		//	                "URL_ENCODED"
		//	              ],
		//	              "type": "string"
		//	            },
		//	            "RequestMethod": {
		//	              "enum": [
		//	                "GET",
		//	                "POST"
		//	              ],
		//	              "type": "string"
		//	            },
		//	            "Scope": {
		//	              "additionalProperties": false,
		//	              "description": "Defines a property configuration for connection types.",
		//	              "properties": {
		//	                "AllowedValues": {
		//	                  "description": "A list of allowed values for the property.",
		//	                  "items": {
		//	                    "type": "string"
		//	                  },
		//	                  "type": "array"
		//	                },
		//	                "DefaultValue": {
		//	                  "description": "The default value for the property.",
		//	                  "type": "string"
		//	                },
		//	                "KeyOverride": {
		//	                  "description": "A key name to use when sending this property in API requests.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "pattern": "^[a-zA-Z0-9_-]+$",
		//	                  "type": "string"
		//	                },
		//	                "Name": {
		//	                  "description": "The name of the property.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "type": "string"
		//	                },
		//	                "PropertyLocation": {
		//	                  "description": "Specifies where this property should be included in REST requests.",
		//	                  "enum": [
		//	                    "HEADER",
		//	                    "BODY",
		//	                    "QUERY_PARAM",
		//	                    "PATH"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "PropertyType": {
		//	                  "description": "The data type of this property.",
		//	                  "enum": [
		//	                    "USER_INPUT",
		//	                    "SECRET",
		//	                    "READ_ONLY",
		//	                    "UNUSED",
		//	                    "SECRET_OR_USER_INPUT"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "Required": {
		//	                  "description": "Indicates whether the property is required.",
		//	                  "type": "boolean"
		//	                }
		//	              },
		//	              "required": [
		//	                "Name",
		//	                "Required",
		//	                "PropertyType"
		//	              ],
		//	              "type": "object"
		//	            },
		//	            "TokenUrl": {
		//	              "additionalProperties": false,
		//	              "description": "Defines a property configuration for connection types.",
		//	              "properties": {
		//	                "AllowedValues": {
		//	                  "description": "A list of allowed values for the property.",
		//	                  "items": {
		//	                    "type": "string"
		//	                  },
		//	                  "type": "array"
		//	                },
		//	                "DefaultValue": {
		//	                  "description": "The default value for the property.",
		//	                  "type": "string"
		//	                },
		//	                "KeyOverride": {
		//	                  "description": "A key name to use when sending this property in API requests.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "pattern": "^[a-zA-Z0-9_-]+$",
		//	                  "type": "string"
		//	                },
		//	                "Name": {
		//	                  "description": "The name of the property.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "type": "string"
		//	                },
		//	                "PropertyLocation": {
		//	                  "description": "Specifies where this property should be included in REST requests.",
		//	                  "enum": [
		//	                    "HEADER",
		//	                    "BODY",
		//	                    "QUERY_PARAM",
		//	                    "PATH"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "PropertyType": {
		//	                  "description": "The data type of this property.",
		//	                  "enum": [
		//	                    "USER_INPUT",
		//	                    "SECRET",
		//	                    "READ_ONLY",
		//	                    "UNUSED",
		//	                    "SECRET_OR_USER_INPUT"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "Required": {
		//	                  "description": "Indicates whether the property is required.",
		//	                  "type": "boolean"
		//	                }
		//	              },
		//	              "required": [
		//	                "Name",
		//	                "Required",
		//	                "PropertyType"
		//	              ],
		//	              "type": "object"
		//	            },
		//	            "TokenUrlParameters": {
		//	              "items": {
		//	                "additionalProperties": false,
		//	                "description": "Defines a property configuration for connection types.",
		//	                "properties": {
		//	                  "AllowedValues": {
		//	                    "description": "A list of allowed values for the property.",
		//	                    "items": {
		//	                      "type": "string"
		//	                    },
		//	                    "type": "array"
		//	                  },
		//	                  "DefaultValue": {
		//	                    "description": "The default value for the property.",
		//	                    "type": "string"
		//	                  },
		//	                  "KeyOverride": {
		//	                    "description": "A key name to use when sending this property in API requests.",
		//	                    "maxLength": 128,
		//	                    "minLength": 1,
		//	                    "pattern": "^[a-zA-Z0-9_-]+$",
		//	                    "type": "string"
		//	                  },
		//	                  "Name": {
		//	                    "description": "The name of the property.",
		//	                    "maxLength": 128,
		//	                    "minLength": 1,
		//	                    "type": "string"
		//	                  },
		//	                  "PropertyLocation": {
		//	                    "description": "Specifies where this property should be included in REST requests.",
		//	                    "enum": [
		//	                      "HEADER",
		//	                      "BODY",
		//	                      "QUERY_PARAM",
		//	                      "PATH"
		//	                    ],
		//	                    "type": "string"
		//	                  },
		//	                  "PropertyType": {
		//	                    "description": "The data type of this property.",
		//	                    "enum": [
		//	                      "USER_INPUT",
		//	                      "SECRET",
		//	                      "READ_ONLY",
		//	                      "UNUSED",
		//	                      "SECRET_OR_USER_INPUT"
		//	                    ],
		//	                    "type": "string"
		//	                  },
		//	                  "Required": {
		//	                    "description": "Indicates whether the property is required.",
		//	                    "type": "boolean"
		//	                  }
		//	                },
		//	                "required": [
		//	                  "Name",
		//	                  "Required",
		//	                  "PropertyType"
		//	                ],
		//	                "type": "object"
		//	              },
		//	              "type": "array"
		//	            }
		//	          },
		//	          "type": "object"
		//	        },
		//	        "JWTBearerProperties": {
		//	          "additionalProperties": false,
		//	          "description": "JWT bearer token configuration.",
		//	          "properties": {
		//	            "ContentType": {
		//	              "enum": [
		//	                "APPLICATION_JSON",
		//	                "URL_ENCODED"
		//	              ],
		//	              "type": "string"
		//	            },
		//	            "JwtToken": {
		//	              "additionalProperties": false,
		//	              "description": "Defines a secret property configuration. SECRET-type properties cannot have DefaultValue or AllowedValues.",
		//	              "properties": {
		//	                "KeyOverride": {
		//	                  "description": "A key name to use when sending this property in API requests.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "pattern": "^[a-zA-Z0-9_-]+$",
		//	                  "type": "string"
		//	                },
		//	                "Name": {
		//	                  "description": "The name of the property.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "type": "string"
		//	                },
		//	                "PropertyLocation": {
		//	                  "description": "Specifies where this property should be included in REST requests.",
		//	                  "enum": [
		//	                    "HEADER",
		//	                    "BODY",
		//	                    "QUERY_PARAM",
		//	                    "PATH"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "PropertyType": {
		//	                  "description": "The data type of this property. Must be SECRET for secret properties.",
		//	                  "enum": [
		//	                    "SECRET"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "Required": {
		//	                  "description": "Indicates whether the property is required.",
		//	                  "type": "boolean"
		//	                }
		//	              },
		//	              "required": [
		//	                "Name",
		//	                "Required",
		//	                "PropertyType"
		//	              ],
		//	              "type": "object"
		//	            },
		//	            "RequestMethod": {
		//	              "enum": [
		//	                "GET",
		//	                "POST"
		//	              ],
		//	              "type": "string"
		//	            },
		//	            "TokenUrl": {
		//	              "additionalProperties": false,
		//	              "description": "Defines a property configuration for connection types.",
		//	              "properties": {
		//	                "AllowedValues": {
		//	                  "description": "A list of allowed values for the property.",
		//	                  "items": {
		//	                    "type": "string"
		//	                  },
		//	                  "type": "array"
		//	                },
		//	                "DefaultValue": {
		//	                  "description": "The default value for the property.",
		//	                  "type": "string"
		//	                },
		//	                "KeyOverride": {
		//	                  "description": "A key name to use when sending this property in API requests.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "pattern": "^[a-zA-Z0-9_-]+$",
		//	                  "type": "string"
		//	                },
		//	                "Name": {
		//	                  "description": "The name of the property.",
		//	                  "maxLength": 128,
		//	                  "minLength": 1,
		//	                  "type": "string"
		//	                },
		//	                "PropertyLocation": {
		//	                  "description": "Specifies where this property should be included in REST requests.",
		//	                  "enum": [
		//	                    "HEADER",
		//	                    "BODY",
		//	                    "QUERY_PARAM",
		//	                    "PATH"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "PropertyType": {
		//	                  "description": "The data type of this property.",
		//	                  "enum": [
		//	                    "USER_INPUT",
		//	                    "SECRET",
		//	                    "READ_ONLY",
		//	                    "UNUSED",
		//	                    "SECRET_OR_USER_INPUT"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "Required": {
		//	                  "description": "Indicates whether the property is required.",
		//	                  "type": "boolean"
		//	                }
		//	              },
		//	              "required": [
		//	                "Name",
		//	                "Required",
		//	                "PropertyType"
		//	              ],
		//	              "type": "object"
		//	            },
		//	            "TokenUrlParameters": {
		//	              "items": {
		//	                "additionalProperties": false,
		//	                "description": "Defines a property configuration for connection types.",
		//	                "properties": {
		//	                  "AllowedValues": {
		//	                    "description": "A list of allowed values for the property.",
		//	                    "items": {
		//	                      "type": "string"
		//	                    },
		//	                    "type": "array"
		//	                  },
		//	                  "DefaultValue": {
		//	                    "description": "The default value for the property.",
		//	                    "type": "string"
		//	                  },
		//	                  "KeyOverride": {
		//	                    "description": "A key name to use when sending this property in API requests.",
		//	                    "maxLength": 128,
		//	                    "minLength": 1,
		//	                    "pattern": "^[a-zA-Z0-9_-]+$",
		//	                    "type": "string"
		//	                  },
		//	                  "Name": {
		//	                    "description": "The name of the property.",
		//	                    "maxLength": 128,
		//	                    "minLength": 1,
		//	                    "type": "string"
		//	                  },
		//	                  "PropertyLocation": {
		//	                    "description": "Specifies where this property should be included in REST requests.",
		//	                    "enum": [
		//	                      "HEADER",
		//	                      "BODY",
		//	                      "QUERY_PARAM",
		//	                      "PATH"
		//	                    ],
		//	                    "type": "string"
		//	                  },
		//	                  "PropertyType": {
		//	                    "description": "The data type of this property.",
		//	                    "enum": [
		//	                      "USER_INPUT",
		//	                      "SECRET",
		//	                      "READ_ONLY",
		//	                      "UNUSED",
		//	                      "SECRET_OR_USER_INPUT"
		//	                    ],
		//	                    "type": "string"
		//	                  },
		//	                  "Required": {
		//	                    "description": "Indicates whether the property is required.",
		//	                    "type": "boolean"
		//	                  }
		//	                },
		//	                "required": [
		//	                  "Name",
		//	                  "Required",
		//	                  "PropertyType"
		//	                ],
		//	                "type": "object"
		//	              },
		//	              "type": "array"
		//	            }
		//	          },
		//	          "type": "object"
		//	        },
		//	        "OAuth2GrantType": {
		//	          "description": "The OAuth2 grant type to use.",
		//	          "enum": [
		//	            "CLIENT_CREDENTIALS",
		//	            "JWT_BEARER",
		//	            "AUTHORIZATION_CODE"
		//	          ],
		//	          "type": "string"
		//	        }
		//	      },
		//	      "required": [
		//	        "OAuth2GrantType"
		//	      ],
		//	      "type": "object"
		//	    }
		//	  },
		//	  "required": [
		//	    "AuthenticationTypes"
		//	  ],
		//	  "type": "object"
		//	}
		"connector_authentication_configuration": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
			Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
				// Property: AuthenticationTypes
				"authentication_types": schema.ListAttribute{ /*START ATTRIBUTE*/
					ElementType: types.StringType,
					Description: "A list of authentication types supported.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: BasicAuthenticationProperties
				"basic_authentication_properties": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
					Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
						// Property: Password
						"password": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
							Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
								// Property: KeyOverride
								"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
									Description: "A key name to use when sending this property in API requests.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: Name
								"name": schema.StringAttribute{ /*START ATTRIBUTE*/
									Description: "The name of the property.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: PropertyLocation
								"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
									Description: "Specifies where this property should be included in REST requests.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: PropertyType
								"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
									Description: "The data type of this property. Must be SECRET for secret properties.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: Required
								"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
									Description: "Indicates whether the property is required.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
							}, /*END SCHEMA*/
							Description: "Defines a secret property configuration. SECRET-type properties cannot have DefaultValue or AllowedValues.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
						// Property: Username
						"username": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
							Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
								// Property: KeyOverride
								"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
									Description: "A key name to use when sending this property in API requests.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: Name
								"name": schema.StringAttribute{ /*START ATTRIBUTE*/
									Description: "The name of the property.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: PropertyLocation
								"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
									Description: "Specifies where this property should be included in REST requests.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: PropertyType
								"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
									Description: "The data type of this property. Must be SECRET for secret properties.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: Required
								"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
									Description: "Indicates whether the property is required.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
							}, /*END SCHEMA*/
							Description: "Defines a secret property configuration. SECRET-type properties cannot have DefaultValue or AllowedValues.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
					}, /*END SCHEMA*/
					Description: "Basic authentication configuration.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: CustomAuthenticationProperties
				"custom_authentication_properties": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
					Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
						// Property: AuthenticationParameters
						"authentication_parameters": schema.ListNestedAttribute{ /*START ATTRIBUTE*/
							NestedObject: schema.NestedAttributeObject{ /*START NESTED OBJECT*/
								Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
									// Property: KeyOverride
									"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
										Description: "A key name to use when sending this property in API requests.",
										Computed:    true,
									}, /*END ATTRIBUTE*/
									// Property: Name
									"name": schema.StringAttribute{ /*START ATTRIBUTE*/
										Description: "The name of the property.",
										Computed:    true,
									}, /*END ATTRIBUTE*/
									// Property: PropertyLocation
									"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
										Description: "Specifies where this property should be included in REST requests.",
										Computed:    true,
									}, /*END ATTRIBUTE*/
									// Property: PropertyType
									"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
										Description: "The data type of this property. Must be SECRET for secret properties.",
										Computed:    true,
									}, /*END ATTRIBUTE*/
									// Property: Required
									"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
										Description: "Indicates whether the property is required.",
										Computed:    true,
									}, /*END ATTRIBUTE*/
								}, /*END SCHEMA*/
							}, /*END NESTED OBJECT*/
							Computed: true,
						}, /*END ATTRIBUTE*/
					}, /*END SCHEMA*/
					Description: "Custom authentication configuration.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: OAuth2Properties
				"o_auth_2_properties": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
					Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
						// Property: AuthorizationCodeProperties
						"authorization_code_properties": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
							Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
								// Property: AuthorizationCode
								"authorization_code": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
									Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
										// Property: AllowedValues
										"allowed_values": schema.ListAttribute{ /*START ATTRIBUTE*/
											ElementType: types.StringType,
											Description: "A list of allowed values for the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: DefaultValue
										"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The default value for the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: KeyOverride
										"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "A key name to use when sending this property in API requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Name
										"name": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The name of the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyLocation
										"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "Specifies where this property should be included in REST requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyType
										"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The data type of this property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Required
										"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
											Description: "Indicates whether the property is required.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
									}, /*END SCHEMA*/
									Description: "Defines a property configuration for connection types.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: AuthorizationCodeUrl
								"authorization_code_url": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
									Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
										// Property: AllowedValues
										"allowed_values": schema.ListAttribute{ /*START ATTRIBUTE*/
											ElementType: types.StringType,
											Description: "A list of allowed values for the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: DefaultValue
										"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The default value for the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: KeyOverride
										"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "A key name to use when sending this property in API requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Name
										"name": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The name of the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyLocation
										"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "Specifies where this property should be included in REST requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyType
										"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The data type of this property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Required
										"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
											Description: "Indicates whether the property is required.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
									}, /*END SCHEMA*/
									Description: "Defines a property configuration for connection types.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: ClientId
								"client_id": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
									Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
										// Property: KeyOverride
										"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "A key name to use when sending this property in API requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Name
										"name": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The name of the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyLocation
										"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "Specifies where this property should be included in REST requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyType
										"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The data type of this property. Must be SECRET for secret properties.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Required
										"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
											Description: "Indicates whether the property is required.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
									}, /*END SCHEMA*/
									Description: "Defines a secret property configuration. SECRET-type properties cannot have DefaultValue or AllowedValues.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: ClientSecret
								"client_secret": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
									Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
										// Property: KeyOverride
										"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "A key name to use when sending this property in API requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Name
										"name": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The name of the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyLocation
										"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "Specifies where this property should be included in REST requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyType
										"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The data type of this property. Must be SECRET for secret properties.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Required
										"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
											Description: "Indicates whether the property is required.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
									}, /*END SCHEMA*/
									Description: "Defines a secret property configuration. SECRET-type properties cannot have DefaultValue or AllowedValues.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: ContentType
								"content_type": schema.StringAttribute{ /*START ATTRIBUTE*/
									Computed: true,
								}, /*END ATTRIBUTE*/
								// Property: Prompt
								"prompt": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
									Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
										// Property: AllowedValues
										"allowed_values": schema.ListAttribute{ /*START ATTRIBUTE*/
											ElementType: types.StringType,
											Description: "A list of allowed values for the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: DefaultValue
										"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The default value for the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: KeyOverride
										"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "A key name to use when sending this property in API requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Name
										"name": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The name of the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyLocation
										"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "Specifies where this property should be included in REST requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyType
										"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The data type of this property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Required
										"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
											Description: "Indicates whether the property is required.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
									}, /*END SCHEMA*/
									Description: "Defines a property configuration for connection types.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: RedirectUri
								"redirect_uri": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
									Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
										// Property: AllowedValues
										"allowed_values": schema.ListAttribute{ /*START ATTRIBUTE*/
											ElementType: types.StringType,
											Description: "A list of allowed values for the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: DefaultValue
										"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The default value for the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: KeyOverride
										"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "A key name to use when sending this property in API requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Name
										"name": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The name of the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyLocation
										"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "Specifies where this property should be included in REST requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyType
										"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The data type of this property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Required
										"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
											Description: "Indicates whether the property is required.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
									}, /*END SCHEMA*/
									Description: "Defines a property configuration for connection types.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: RequestMethod
								"request_method": schema.StringAttribute{ /*START ATTRIBUTE*/
									Computed: true,
								}, /*END ATTRIBUTE*/
								// Property: Scope
								"scope": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
									Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
										// Property: AllowedValues
										"allowed_values": schema.ListAttribute{ /*START ATTRIBUTE*/
											ElementType: types.StringType,
											Description: "A list of allowed values for the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: DefaultValue
										"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The default value for the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: KeyOverride
										"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "A key name to use when sending this property in API requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Name
										"name": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The name of the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyLocation
										"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "Specifies where this property should be included in REST requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyType
										"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The data type of this property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Required
										"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
											Description: "Indicates whether the property is required.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
									}, /*END SCHEMA*/
									Description: "Defines a property configuration for connection types.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: TokenUrl
								"token_url": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
									Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
										// Property: AllowedValues
										"allowed_values": schema.ListAttribute{ /*START ATTRIBUTE*/
											ElementType: types.StringType,
											Description: "A list of allowed values for the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: DefaultValue
										"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The default value for the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: KeyOverride
										"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "A key name to use when sending this property in API requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Name
										"name": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The name of the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyLocation
										"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "Specifies where this property should be included in REST requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyType
										"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The data type of this property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Required
										"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
											Description: "Indicates whether the property is required.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
									}, /*END SCHEMA*/
									Description: "Defines a property configuration for connection types.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: TokenUrlParameters
								"token_url_parameters": schema.ListNestedAttribute{ /*START ATTRIBUTE*/
									NestedObject: schema.NestedAttributeObject{ /*START NESTED OBJECT*/
										Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
											// Property: AllowedValues
											"allowed_values": schema.ListAttribute{ /*START ATTRIBUTE*/
												ElementType: types.StringType,
												Description: "A list of allowed values for the property.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: DefaultValue
											"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
												Description: "The default value for the property.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: KeyOverride
											"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
												Description: "A key name to use when sending this property in API requests.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: Name
											"name": schema.StringAttribute{ /*START ATTRIBUTE*/
												Description: "The name of the property.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: PropertyLocation
											"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
												Description: "Specifies where this property should be included in REST requests.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: PropertyType
											"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
												Description: "The data type of this property.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: Required
											"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
												Description: "Indicates whether the property is required.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
										}, /*END SCHEMA*/
									}, /*END NESTED OBJECT*/
									Computed: true,
								}, /*END ATTRIBUTE*/
							}, /*END SCHEMA*/
							Description: "OAuth2 authorization code configuration.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
						// Property: ClientCredentialsProperties
						"client_credentials_properties": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
							Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
								// Property: ClientId
								"client_id": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
									Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
										// Property: KeyOverride
										"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "A key name to use when sending this property in API requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Name
										"name": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The name of the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyLocation
										"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "Specifies where this property should be included in REST requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyType
										"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The data type of this property. Must be SECRET for secret properties.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Required
										"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
											Description: "Indicates whether the property is required.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
									}, /*END SCHEMA*/
									Description: "Defines a secret property configuration. SECRET-type properties cannot have DefaultValue or AllowedValues.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: ClientSecret
								"client_secret": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
									Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
										// Property: KeyOverride
										"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "A key name to use when sending this property in API requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Name
										"name": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The name of the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyLocation
										"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "Specifies where this property should be included in REST requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyType
										"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The data type of this property. Must be SECRET for secret properties.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Required
										"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
											Description: "Indicates whether the property is required.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
									}, /*END SCHEMA*/
									Description: "Defines a secret property configuration. SECRET-type properties cannot have DefaultValue or AllowedValues.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: ContentType
								"content_type": schema.StringAttribute{ /*START ATTRIBUTE*/
									Computed: true,
								}, /*END ATTRIBUTE*/
								// Property: RequestMethod
								"request_method": schema.StringAttribute{ /*START ATTRIBUTE*/
									Computed: true,
								}, /*END ATTRIBUTE*/
								// Property: Scope
								"scope": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
									Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
										// Property: AllowedValues
										"allowed_values": schema.ListAttribute{ /*START ATTRIBUTE*/
											ElementType: types.StringType,
											Description: "A list of allowed values for the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: DefaultValue
										"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The default value for the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: KeyOverride
										"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "A key name to use when sending this property in API requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Name
										"name": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The name of the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyLocation
										"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "Specifies where this property should be included in REST requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyType
										"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The data type of this property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Required
										"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
											Description: "Indicates whether the property is required.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
									}, /*END SCHEMA*/
									Description: "Defines a property configuration for connection types.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: TokenUrl
								"token_url": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
									Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
										// Property: AllowedValues
										"allowed_values": schema.ListAttribute{ /*START ATTRIBUTE*/
											ElementType: types.StringType,
											Description: "A list of allowed values for the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: DefaultValue
										"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The default value for the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: KeyOverride
										"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "A key name to use when sending this property in API requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Name
										"name": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The name of the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyLocation
										"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "Specifies where this property should be included in REST requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyType
										"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The data type of this property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Required
										"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
											Description: "Indicates whether the property is required.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
									}, /*END SCHEMA*/
									Description: "Defines a property configuration for connection types.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: TokenUrlParameters
								"token_url_parameters": schema.ListNestedAttribute{ /*START ATTRIBUTE*/
									NestedObject: schema.NestedAttributeObject{ /*START NESTED OBJECT*/
										Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
											// Property: AllowedValues
											"allowed_values": schema.ListAttribute{ /*START ATTRIBUTE*/
												ElementType: types.StringType,
												Description: "A list of allowed values for the property.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: DefaultValue
											"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
												Description: "The default value for the property.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: KeyOverride
											"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
												Description: "A key name to use when sending this property in API requests.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: Name
											"name": schema.StringAttribute{ /*START ATTRIBUTE*/
												Description: "The name of the property.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: PropertyLocation
											"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
												Description: "Specifies where this property should be included in REST requests.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: PropertyType
											"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
												Description: "The data type of this property.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: Required
											"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
												Description: "Indicates whether the property is required.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
										}, /*END SCHEMA*/
									}, /*END NESTED OBJECT*/
									Computed: true,
								}, /*END ATTRIBUTE*/
							}, /*END SCHEMA*/
							Description: "OAuth2 client credentials configuration.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
						// Property: JWTBearerProperties
						"jwt_bearer_properties": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
							Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
								// Property: ContentType
								"content_type": schema.StringAttribute{ /*START ATTRIBUTE*/
									Computed: true,
								}, /*END ATTRIBUTE*/
								// Property: JwtToken
								"jwt_token": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
									Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
										// Property: KeyOverride
										"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "A key name to use when sending this property in API requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Name
										"name": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The name of the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyLocation
										"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "Specifies where this property should be included in REST requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyType
										"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The data type of this property. Must be SECRET for secret properties.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Required
										"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
											Description: "Indicates whether the property is required.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
									}, /*END SCHEMA*/
									Description: "Defines a secret property configuration. SECRET-type properties cannot have DefaultValue or AllowedValues.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: RequestMethod
								"request_method": schema.StringAttribute{ /*START ATTRIBUTE*/
									Computed: true,
								}, /*END ATTRIBUTE*/
								// Property: TokenUrl
								"token_url": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
									Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
										// Property: AllowedValues
										"allowed_values": schema.ListAttribute{ /*START ATTRIBUTE*/
											ElementType: types.StringType,
											Description: "A list of allowed values for the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: DefaultValue
										"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The default value for the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: KeyOverride
										"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "A key name to use when sending this property in API requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Name
										"name": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The name of the property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyLocation
										"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "Specifies where this property should be included in REST requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: PropertyType
										"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The data type of this property.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Required
										"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
											Description: "Indicates whether the property is required.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
									}, /*END SCHEMA*/
									Description: "Defines a property configuration for connection types.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: TokenUrlParameters
								"token_url_parameters": schema.ListNestedAttribute{ /*START ATTRIBUTE*/
									NestedObject: schema.NestedAttributeObject{ /*START NESTED OBJECT*/
										Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
											// Property: AllowedValues
											"allowed_values": schema.ListAttribute{ /*START ATTRIBUTE*/
												ElementType: types.StringType,
												Description: "A list of allowed values for the property.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: DefaultValue
											"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
												Description: "The default value for the property.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: KeyOverride
											"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
												Description: "A key name to use when sending this property in API requests.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: Name
											"name": schema.StringAttribute{ /*START ATTRIBUTE*/
												Description: "The name of the property.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: PropertyLocation
											"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
												Description: "Specifies where this property should be included in REST requests.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: PropertyType
											"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
												Description: "The data type of this property.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: Required
											"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
												Description: "Indicates whether the property is required.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
										}, /*END SCHEMA*/
									}, /*END NESTED OBJECT*/
									Computed: true,
								}, /*END ATTRIBUTE*/
							}, /*END SCHEMA*/
							Description: "JWT bearer token configuration.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
						// Property: OAuth2GrantType
						"o_auth_2_grant_type": schema.StringAttribute{ /*START ATTRIBUTE*/
							Description: "The OAuth2 grant type to use.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
					}, /*END SCHEMA*/
					Description: "OAuth2 configuration container.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
			}, /*END SCHEMA*/
			Description: "Configuration that defines supported authentication types and required properties.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: Description
		// CloudFormation resource type schema:
		//
		//	{
		//	  "description": "A description of the connection type.",
		//	  "maxLength": 1024,
		//	  "minLength": 0,
		//	  "type": "string"
		//	}
		"description": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "A description of the connection type.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: IntegrationType
		// CloudFormation resource type schema:
		//
		//	{
		//	  "description": "The integration type for the connection. Currently only REST is supported.",
		//	  "enum": [
		//	    "REST"
		//	  ],
		//	  "type": "string"
		//	}
		"integration_type": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "The integration type for the connection. Currently only REST is supported.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: RestConfiguration
		// CloudFormation resource type schema:
		//
		//	{
		//	  "additionalProperties": false,
		//	  "description": "Configuration for HTTP request and response handling.",
		//	  "properties": {
		//	    "EntityConfigurations": {
		//	      "additionalProperties": false,
		//	      "description": "A map of entity configurations.",
		//	      "patternProperties": {
		//	        "": {
		//	          "additionalProperties": false,
		//	          "description": "Configuration for interacting with a specific data entity.",
		//	          "properties": {
		//	            "Schema": {
		//	              "additionalProperties": false,
		//	              "description": "The schema definition for this entity.",
		//	              "patternProperties": {
		//	                "": {
		//	                  "additionalProperties": false,
		//	                  "description": "Defines a field in an entity schema.",
		//	                  "properties": {
		//	                    "FieldDataType": {
		//	                      "description": "The data type of the field.",
		//	                      "enum": [
		//	                        "INT",
		//	                        "SMALLINT",
		//	                        "BIGINT",
		//	                        "FLOAT",
		//	                        "LONG",
		//	                        "DATE",
		//	                        "BOOLEAN",
		//	                        "MAP",
		//	                        "ARRAY",
		//	                        "STRING",
		//	                        "TIMESTAMP",
		//	                        "DECIMAL",
		//	                        "BYTE",
		//	                        "SHORT",
		//	                        "DOUBLE",
		//	                        "STRUCT",
		//	                        "BINARY",
		//	                        "UNION"
		//	                      ],
		//	                      "type": "string"
		//	                    },
		//	                    "FilterOverrides": {
		//	                      "additionalProperties": false,
		//	                      "description": "Configuration that defines per-field overrides for filter behavior, allowing individual fields to customize how filter operations are applied.",
		//	                      "properties": {
		//	                        "BetweenConfiguration": {
		//	                          "additionalProperties": false,
		//	                          "description": "Configuration that defines how BETWEEN range filter operations are translated into REST API request parameters.",
		//	                          "properties": {
		//	                            "HighBoundKey": {
		//	                              "description": "The parameter name used for the upper bound value in a BETWEEN filter operation.",
		//	                              "type": "string"
		//	                            },
		//	                            "LowBoundKey": {
		//	                              "description": "The parameter name used for the lower bound value in a BETWEEN filter operation.",
		//	                              "type": "string"
		//	                            },
		//	                            "Template": {
		//	                              "description": "A template string for constructing the BETWEEN filter expression.",
		//	                              "type": "string"
		//	                            }
		//	                          },
		//	                          "type": "object"
		//	                        },
		//	                        "DateTimeFormat": {
		//	                          "description": "The date and time format for filter expressions on this field, overriding the global DateTimeFormat.",
		//	                          "type": "string"
		//	                        },
		//	                        "FieldName": {
		//	                          "description": "An override for the field name to use in filter expressions, if different from the schema field name.",
		//	                          "type": "string"
		//	                        },
		//	                        "OperatorMappings": {
		//	                          "additionalProperties": false,
		//	                          "description": "A map of logical filter operators to their field-specific API representations, overriding the global operator mappings.",
		//	                          "patternProperties": {
		//	                            "": {
		//	                              "type": "string"
		//	                            }
		//	                          },
		//	                          "type": "object"
		//	                        }
		//	                      },
		//	                      "type": "object"
		//	                    },
		//	                    "IsNullable": {
		//	                      "description": "Indicates whether this field can contain null values.",
		//	                      "type": "boolean"
		//	                    },
		//	                    "IsOrderable": {
		//	                      "description": "Indicates whether this field can be used for ordering results.",
		//	                      "type": "boolean"
		//	                    },
		//	                    "IsPartitionable": {
		//	                      "description": "Indicates whether this field can be used for partitioning queries to the data source.",
		//	                      "type": "boolean"
		//	                    },
		//	                    "IsQueryable": {
		//	                      "description": "Indicates whether this field can be used in filter predicates when querying data.",
		//	                      "type": "boolean"
		//	                    },
		//	                    "Name": {
		//	                      "description": "The name of the field.",
		//	                      "type": "string"
		//	                    },
		//	                    "ResponseDateFormat": {
		//	                      "description": "The format pattern for parsing date values from API responses. Accepts Java DateTimeFormatter patterns, EPOCH_SECONDS, or EPOCH_MILLIS.",
		//	                      "type": "string"
		//	                    }
		//	                  },
		//	                  "required": [
		//	                    "Name",
		//	                    "FieldDataType"
		//	                  ],
		//	                  "type": "object"
		//	                }
		//	              },
		//	              "type": "object"
		//	            },
		//	            "SourceConfiguration": {
		//	              "additionalProperties": false,
		//	              "description": "Configuration that defines how to make requests to endpoints.",
		//	              "properties": {
		//	                "FilterConfiguration": {
		//	                  "additionalProperties": false,
		//	                  "description": "Configuration that defines how filter predicates are applied to REST API requests, supporting both query parameter and filter string strategies.",
		//	                  "properties": {
		//	                    "BetweenConfiguration": {
		//	                      "additionalProperties": false,
		//	                      "description": "Configuration that defines how BETWEEN range filter operations are translated into REST API request parameters.",
		//	                      "properties": {
		//	                        "HighBoundKey": {
		//	                          "description": "The parameter name used for the upper bound value in a BETWEEN filter operation.",
		//	                          "type": "string"
		//	                        },
		//	                        "LowBoundKey": {
		//	                          "description": "The parameter name used for the lower bound value in a BETWEEN filter operation.",
		//	                          "type": "string"
		//	                        },
		//	                        "Template": {
		//	                          "description": "A template string for constructing the BETWEEN filter expression.",
		//	                          "type": "string"
		//	                        }
		//	                      },
		//	                      "type": "object"
		//	                    },
		//	                    "DateTimeFormat": {
		//	                      "description": "The global date and time format for filter expressions.",
		//	                      "type": "string"
		//	                    },
		//	                    "FilterMode": {
		//	                      "description": "The strategy for applying filters to requests.",
		//	                      "enum": [
		//	                        "QUERY_PARAMS",
		//	                        "FILTER_STRING"
		//	                      ],
		//	                      "type": "string"
		//	                    },
		//	                    "FilterStringConfiguration": {
		//	                      "additionalProperties": false,
		//	                      "description": "Configuration for constructing filter expression strings when using the FILTER_STRING filter mode.",
		//	                      "properties": {
		//	                        "QueryParameterName": {
		//	                          "description": "The query parameter name used to send the constructed filter expression string in API requests.",
		//	                          "type": "string"
		//	                        },
		//	                        "QuoteCharacter": {
		//	                          "description": "The character used to quote values when QuoteStringValues is true. Defaults to double quotes if not specified.",
		//	                          "type": "string"
		//	                        },
		//	                        "QuoteStringValues": {
		//	                          "description": "Indicates whether string and date values should be wrapped with a quote character in the filter expression.",
		//	                          "type": "boolean"
		//	                        }
		//	                      },
		//	                      "required": [
		//	                        "QueryParameterName"
		//	                      ],
		//	                      "type": "object"
		//	                    },
		//	                    "OperatorMappings": {
		//	                      "additionalProperties": false,
		//	                      "description": "A map of logical filter operators to their API-specific string representations.",
		//	                      "patternProperties": {
		//	                        "": {
		//	                          "type": "string"
		//	                        }
		//	                      },
		//	                      "type": "object"
		//	                    },
		//	                    "StripQuotes": {
		//	                      "description": "Indicates whether surrounding double quotes should be stripped from filter values before processing.",
		//	                      "type": "boolean"
		//	                    }
		//	                  },
		//	                  "required": [
		//	                    "FilterMode"
		//	                  ],
		//	                  "type": "object"
		//	                },
		//	                "PaginationConfiguration": {
		//	                  "additionalProperties": false,
		//	                  "description": "Configuration for handling paginated responses.",
		//	                  "properties": {
		//	                    "CursorConfiguration": {
		//	                      "additionalProperties": false,
		//	                      "description": "Cursor-based pagination configuration.",
		//	                      "properties": {
		//	                        "LimitParameter": {
		//	                          "additionalProperties": false,
		//	                          "description": "Parameter extraction configuration.",
		//	                          "properties": {
		//	                            "DefaultValue": {
		//	                              "description": "The default value.",
		//	                              "maxLength": 1024,
		//	                              "minLength": 1,
		//	                              "type": "string"
		//	                            },
		//	                            "Key": {
		//	                              "description": "The parameter key name.",
		//	                              "maxLength": 128,
		//	                              "minLength": 1,
		//	                              "pattern": "^[a-zA-Z0-9_-]+$",
		//	                              "type": "string"
		//	                            },
		//	                            "PropertyLocation": {
		//	                              "description": "Specifies where to place the parameter in requests.",
		//	                              "enum": [
		//	                                "HEADER",
		//	                                "BODY",
		//	                                "QUERY_PARAM",
		//	                                "PATH"
		//	                              ],
		//	                              "type": "string"
		//	                            },
		//	                            "Value": {
		//	                              "additionalProperties": false,
		//	                              "description": "Defines how to extract values from HTTP responses.",
		//	                              "properties": {
		//	                                "ContentPath": {
		//	                                  "description": "A JSON path expression to extract a value from response body.",
		//	                                  "maxLength": 512,
		//	                                  "minLength": 1,
		//	                                  "pattern": "^\\$(\\.[a-zA-Z0-9_.@\\[\\]\\(\\)-]+)*$",
		//	                                  "type": "string"
		//	                                },
		//	                                "HeaderKey": {
		//	                                  "description": "The name of an HTTP response header from which to extract the value.",
		//	                                  "maxLength": 128,
		//	                                  "minLength": 1,
		//	                                  "pattern": "^[a-zA-Z0-9_-]+$",
		//	                                  "type": "string"
		//	                                }
		//	                              },
		//	                              "type": "object"
		//	                            }
		//	                          },
		//	                          "type": "object"
		//	                        },
		//	                        "NextPage": {
		//	                          "additionalProperties": false,
		//	                          "description": "Parameter extraction configuration.",
		//	                          "properties": {
		//	                            "DefaultValue": {
		//	                              "description": "The default value.",
		//	                              "maxLength": 1024,
		//	                              "minLength": 1,
		//	                              "type": "string"
		//	                            },
		//	                            "Key": {
		//	                              "description": "The parameter key name.",
		//	                              "maxLength": 128,
		//	                              "minLength": 1,
		//	                              "pattern": "^[a-zA-Z0-9_-]+$",
		//	                              "type": "string"
		//	                            },
		//	                            "PropertyLocation": {
		//	                              "description": "Specifies where to place the parameter in requests.",
		//	                              "enum": [
		//	                                "HEADER",
		//	                                "BODY",
		//	                                "QUERY_PARAM",
		//	                                "PATH"
		//	                              ],
		//	                              "type": "string"
		//	                            },
		//	                            "Value": {
		//	                              "additionalProperties": false,
		//	                              "description": "Defines how to extract values from HTTP responses.",
		//	                              "properties": {
		//	                                "ContentPath": {
		//	                                  "description": "A JSON path expression to extract a value from response body.",
		//	                                  "maxLength": 512,
		//	                                  "minLength": 1,
		//	                                  "pattern": "^\\$(\\.[a-zA-Z0-9_.@\\[\\]\\(\\)-]+)*$",
		//	                                  "type": "string"
		//	                                },
		//	                                "HeaderKey": {
		//	                                  "description": "The name of an HTTP response header from which to extract the value.",
		//	                                  "maxLength": 128,
		//	                                  "minLength": 1,
		//	                                  "pattern": "^[a-zA-Z0-9_-]+$",
		//	                                  "type": "string"
		//	                                }
		//	                              },
		//	                              "type": "object"
		//	                            }
		//	                          },
		//	                          "type": "object"
		//	                        }
		//	                      },
		//	                      "required": [
		//	                        "NextPage"
		//	                      ],
		//	                      "type": "object"
		//	                    },
		//	                    "OffsetConfiguration": {
		//	                      "additionalProperties": false,
		//	                      "description": "Offset-based pagination configuration.",
		//	                      "properties": {
		//	                        "LimitParameter": {
		//	                          "additionalProperties": false,
		//	                          "description": "Parameter extraction configuration.",
		//	                          "properties": {
		//	                            "DefaultValue": {
		//	                              "description": "The default value.",
		//	                              "maxLength": 1024,
		//	                              "minLength": 1,
		//	                              "type": "string"
		//	                            },
		//	                            "Key": {
		//	                              "description": "The parameter key name.",
		//	                              "maxLength": 128,
		//	                              "minLength": 1,
		//	                              "pattern": "^[a-zA-Z0-9_-]+$",
		//	                              "type": "string"
		//	                            },
		//	                            "PropertyLocation": {
		//	                              "description": "Specifies where to place the parameter in requests.",
		//	                              "enum": [
		//	                                "HEADER",
		//	                                "BODY",
		//	                                "QUERY_PARAM",
		//	                                "PATH"
		//	                              ],
		//	                              "type": "string"
		//	                            },
		//	                            "Value": {
		//	                              "additionalProperties": false,
		//	                              "description": "Defines how to extract values from HTTP responses.",
		//	                              "properties": {
		//	                                "ContentPath": {
		//	                                  "description": "A JSON path expression to extract a value from response body.",
		//	                                  "maxLength": 512,
		//	                                  "minLength": 1,
		//	                                  "pattern": "^\\$(\\.[a-zA-Z0-9_.@\\[\\]\\(\\)-]+)*$",
		//	                                  "type": "string"
		//	                                },
		//	                                "HeaderKey": {
		//	                                  "description": "The name of an HTTP response header from which to extract the value.",
		//	                                  "maxLength": 128,
		//	                                  "minLength": 1,
		//	                                  "pattern": "^[a-zA-Z0-9_-]+$",
		//	                                  "type": "string"
		//	                                }
		//	                              },
		//	                              "type": "object"
		//	                            }
		//	                          },
		//	                          "type": "object"
		//	                        },
		//	                        "OffsetParameter": {
		//	                          "additionalProperties": false,
		//	                          "description": "Parameter extraction configuration.",
		//	                          "properties": {
		//	                            "DefaultValue": {
		//	                              "description": "The default value.",
		//	                              "maxLength": 1024,
		//	                              "minLength": 1,
		//	                              "type": "string"
		//	                            },
		//	                            "Key": {
		//	                              "description": "The parameter key name.",
		//	                              "maxLength": 128,
		//	                              "minLength": 1,
		//	                              "pattern": "^[a-zA-Z0-9_-]+$",
		//	                              "type": "string"
		//	                            },
		//	                            "PropertyLocation": {
		//	                              "description": "Specifies where to place the parameter in requests.",
		//	                              "enum": [
		//	                                "HEADER",
		//	                                "BODY",
		//	                                "QUERY_PARAM",
		//	                                "PATH"
		//	                              ],
		//	                              "type": "string"
		//	                            },
		//	                            "Value": {
		//	                              "additionalProperties": false,
		//	                              "description": "Defines how to extract values from HTTP responses.",
		//	                              "properties": {
		//	                                "ContentPath": {
		//	                                  "description": "A JSON path expression to extract a value from response body.",
		//	                                  "maxLength": 512,
		//	                                  "minLength": 1,
		//	                                  "pattern": "^\\$(\\.[a-zA-Z0-9_.@\\[\\]\\(\\)-]+)*$",
		//	                                  "type": "string"
		//	                                },
		//	                                "HeaderKey": {
		//	                                  "description": "The name of an HTTP response header from which to extract the value.",
		//	                                  "maxLength": 128,
		//	                                  "minLength": 1,
		//	                                  "pattern": "^[a-zA-Z0-9_-]+$",
		//	                                  "type": "string"
		//	                                }
		//	                              },
		//	                              "type": "object"
		//	                            }
		//	                          },
		//	                          "type": "object"
		//	                        }
		//	                      },
		//	                      "required": [
		//	                        "OffsetParameter",
		//	                        "LimitParameter"
		//	                      ],
		//	                      "type": "object"
		//	                    }
		//	                  },
		//	                  "type": "object"
		//	                },
		//	                "RequestMethod": {
		//	                  "description": "The HTTP method to use.",
		//	                  "enum": [
		//	                    "GET",
		//	                    "POST"
		//	                  ],
		//	                  "type": "string"
		//	                },
		//	                "RequestParameters": {
		//	                  "description": "Request parameters configuration.",
		//	                  "items": {
		//	                    "additionalProperties": false,
		//	                    "description": "Defines a property configuration for connection types.",
		//	                    "properties": {
		//	                      "AllowedValues": {
		//	                        "description": "A list of allowed values for the property.",
		//	                        "items": {
		//	                          "type": "string"
		//	                        },
		//	                        "type": "array"
		//	                      },
		//	                      "DefaultValue": {
		//	                        "description": "The default value for the property.",
		//	                        "type": "string"
		//	                      },
		//	                      "KeyOverride": {
		//	                        "description": "A key name to use when sending this property in API requests.",
		//	                        "maxLength": 128,
		//	                        "minLength": 1,
		//	                        "pattern": "^[a-zA-Z0-9_-]+$",
		//	                        "type": "string"
		//	                      },
		//	                      "Name": {
		//	                        "description": "The name of the property.",
		//	                        "maxLength": 128,
		//	                        "minLength": 1,
		//	                        "type": "string"
		//	                      },
		//	                      "PropertyLocation": {
		//	                        "description": "Specifies where this property should be included in REST requests.",
		//	                        "enum": [
		//	                          "HEADER",
		//	                          "BODY",
		//	                          "QUERY_PARAM",
		//	                          "PATH"
		//	                        ],
		//	                        "type": "string"
		//	                      },
		//	                      "PropertyType": {
		//	                        "description": "The data type of this property.",
		//	                        "enum": [
		//	                          "USER_INPUT",
		//	                          "SECRET",
		//	                          "READ_ONLY",
		//	                          "UNUSED",
		//	                          "SECRET_OR_USER_INPUT"
		//	                        ],
		//	                        "type": "string"
		//	                      },
		//	                      "Required": {
		//	                        "description": "Indicates whether the property is required.",
		//	                        "type": "boolean"
		//	                      }
		//	                    },
		//	                    "required": [
		//	                      "Name",
		//	                      "Required",
		//	                      "PropertyType"
		//	                    ],
		//	                    "type": "object"
		//	                  },
		//	                  "type": "array"
		//	                },
		//	                "RequestPath": {
		//	                  "description": "The URL path for the REST endpoint.",
		//	                  "maxLength": 512,
		//	                  "minLength": 1,
		//	                  "pattern": "^/[a-zA-Z0-9._~:/?#\\[\\]@!$\u0026'()*+,;={}-]*$",
		//	                  "type": "string"
		//	                },
		//	                "ResponseConfiguration": {
		//	                  "additionalProperties": false,
		//	                  "description": "Configuration for parsing JSON responses from REST API calls.",
		//	                  "properties": {
		//	                    "ErrorPath": {
		//	                      "description": "JSON path expression for error information location.",
		//	                      "maxLength": 512,
		//	                      "minLength": 1,
		//	                      "pattern": "^\\$(\\.[a-zA-Z0-9_.@\\[\\]\\(\\)-]+)*$",
		//	                      "type": "string"
		//	                    },
		//	                    "ResultPath": {
		//	                      "description": "JSON path expression for result data location.",
		//	                      "maxLength": 512,
		//	                      "minLength": 1,
		//	                      "pattern": "^\\$(\\.[a-zA-Z0-9_.@\\[\\]\\(\\)-]+)*$",
		//	                      "type": "string"
		//	                    }
		//	                  },
		//	                  "required": [
		//	                    "ResultPath"
		//	                  ],
		//	                  "type": "object"
		//	                }
		//	              },
		//	              "type": "object"
		//	            }
		//	          },
		//	          "type": "object"
		//	        }
		//	      },
		//	      "type": "object"
		//	    },
		//	    "GlobalSourceConfiguration": {
		//	      "additionalProperties": false,
		//	      "description": "Configuration that defines how to make requests to endpoints.",
		//	      "properties": {
		//	        "FilterConfiguration": {
		//	          "additionalProperties": false,
		//	          "description": "Configuration that defines how filter predicates are applied to REST API requests, supporting both query parameter and filter string strategies.",
		//	          "properties": {
		//	            "BetweenConfiguration": {
		//	              "additionalProperties": false,
		//	              "description": "Configuration that defines how BETWEEN range filter operations are translated into REST API request parameters.",
		//	              "properties": {
		//	                "HighBoundKey": {
		//	                  "description": "The parameter name used for the upper bound value in a BETWEEN filter operation.",
		//	                  "type": "string"
		//	                },
		//	                "LowBoundKey": {
		//	                  "description": "The parameter name used for the lower bound value in a BETWEEN filter operation.",
		//	                  "type": "string"
		//	                },
		//	                "Template": {
		//	                  "description": "A template string for constructing the BETWEEN filter expression.",
		//	                  "type": "string"
		//	                }
		//	              },
		//	              "type": "object"
		//	            },
		//	            "DateTimeFormat": {
		//	              "description": "The global date and time format for filter expressions.",
		//	              "type": "string"
		//	            },
		//	            "FilterMode": {
		//	              "description": "The strategy for applying filters to requests.",
		//	              "enum": [
		//	                "QUERY_PARAMS",
		//	                "FILTER_STRING"
		//	              ],
		//	              "type": "string"
		//	            },
		//	            "FilterStringConfiguration": {
		//	              "additionalProperties": false,
		//	              "description": "Configuration for constructing filter expression strings when using the FILTER_STRING filter mode.",
		//	              "properties": {
		//	                "QueryParameterName": {
		//	                  "description": "The query parameter name used to send the constructed filter expression string in API requests.",
		//	                  "type": "string"
		//	                },
		//	                "QuoteCharacter": {
		//	                  "description": "The character used to quote values when QuoteStringValues is true. Defaults to double quotes if not specified.",
		//	                  "type": "string"
		//	                },
		//	                "QuoteStringValues": {
		//	                  "description": "Indicates whether string and date values should be wrapped with a quote character in the filter expression.",
		//	                  "type": "boolean"
		//	                }
		//	              },
		//	              "required": [
		//	                "QueryParameterName"
		//	              ],
		//	              "type": "object"
		//	            },
		//	            "OperatorMappings": {
		//	              "additionalProperties": false,
		//	              "description": "A map of logical filter operators to their API-specific string representations.",
		//	              "patternProperties": {
		//	                "": {
		//	                  "type": "string"
		//	                }
		//	              },
		//	              "type": "object"
		//	            },
		//	            "StripQuotes": {
		//	              "description": "Indicates whether surrounding double quotes should be stripped from filter values before processing.",
		//	              "type": "boolean"
		//	            }
		//	          },
		//	          "required": [
		//	            "FilterMode"
		//	          ],
		//	          "type": "object"
		//	        },
		//	        "PaginationConfiguration": {
		//	          "additionalProperties": false,
		//	          "description": "Configuration for handling paginated responses.",
		//	          "properties": {
		//	            "CursorConfiguration": {
		//	              "additionalProperties": false,
		//	              "description": "Cursor-based pagination configuration.",
		//	              "properties": {
		//	                "LimitParameter": {
		//	                  "additionalProperties": false,
		//	                  "description": "Parameter extraction configuration.",
		//	                  "properties": {
		//	                    "DefaultValue": {
		//	                      "description": "The default value.",
		//	                      "maxLength": 1024,
		//	                      "minLength": 1,
		//	                      "type": "string"
		//	                    },
		//	                    "Key": {
		//	                      "description": "The parameter key name.",
		//	                      "maxLength": 128,
		//	                      "minLength": 1,
		//	                      "pattern": "^[a-zA-Z0-9_-]+$",
		//	                      "type": "string"
		//	                    },
		//	                    "PropertyLocation": {
		//	                      "description": "Specifies where to place the parameter in requests.",
		//	                      "enum": [
		//	                        "HEADER",
		//	                        "BODY",
		//	                        "QUERY_PARAM",
		//	                        "PATH"
		//	                      ],
		//	                      "type": "string"
		//	                    },
		//	                    "Value": {
		//	                      "additionalProperties": false,
		//	                      "description": "Defines how to extract values from HTTP responses.",
		//	                      "properties": {
		//	                        "ContentPath": {
		//	                          "description": "A JSON path expression to extract a value from response body.",
		//	                          "maxLength": 512,
		//	                          "minLength": 1,
		//	                          "pattern": "^\\$(\\.[a-zA-Z0-9_.@\\[\\]\\(\\)-]+)*$",
		//	                          "type": "string"
		//	                        },
		//	                        "HeaderKey": {
		//	                          "description": "The name of an HTTP response header from which to extract the value.",
		//	                          "maxLength": 128,
		//	                          "minLength": 1,
		//	                          "pattern": "^[a-zA-Z0-9_-]+$",
		//	                          "type": "string"
		//	                        }
		//	                      },
		//	                      "type": "object"
		//	                    }
		//	                  },
		//	                  "type": "object"
		//	                },
		//	                "NextPage": {
		//	                  "additionalProperties": false,
		//	                  "description": "Parameter extraction configuration.",
		//	                  "properties": {
		//	                    "DefaultValue": {
		//	                      "description": "The default value.",
		//	                      "maxLength": 1024,
		//	                      "minLength": 1,
		//	                      "type": "string"
		//	                    },
		//	                    "Key": {
		//	                      "description": "The parameter key name.",
		//	                      "maxLength": 128,
		//	                      "minLength": 1,
		//	                      "pattern": "^[a-zA-Z0-9_-]+$",
		//	                      "type": "string"
		//	                    },
		//	                    "PropertyLocation": {
		//	                      "description": "Specifies where to place the parameter in requests.",
		//	                      "enum": [
		//	                        "HEADER",
		//	                        "BODY",
		//	                        "QUERY_PARAM",
		//	                        "PATH"
		//	                      ],
		//	                      "type": "string"
		//	                    },
		//	                    "Value": {
		//	                      "additionalProperties": false,
		//	                      "description": "Defines how to extract values from HTTP responses.",
		//	                      "properties": {
		//	                        "ContentPath": {
		//	                          "description": "A JSON path expression to extract a value from response body.",
		//	                          "maxLength": 512,
		//	                          "minLength": 1,
		//	                          "pattern": "^\\$(\\.[a-zA-Z0-9_.@\\[\\]\\(\\)-]+)*$",
		//	                          "type": "string"
		//	                        },
		//	                        "HeaderKey": {
		//	                          "description": "The name of an HTTP response header from which to extract the value.",
		//	                          "maxLength": 128,
		//	                          "minLength": 1,
		//	                          "pattern": "^[a-zA-Z0-9_-]+$",
		//	                          "type": "string"
		//	                        }
		//	                      },
		//	                      "type": "object"
		//	                    }
		//	                  },
		//	                  "type": "object"
		//	                }
		//	              },
		//	              "required": [
		//	                "NextPage"
		//	              ],
		//	              "type": "object"
		//	            },
		//	            "OffsetConfiguration": {
		//	              "additionalProperties": false,
		//	              "description": "Offset-based pagination configuration.",
		//	              "properties": {
		//	                "LimitParameter": {
		//	                  "additionalProperties": false,
		//	                  "description": "Parameter extraction configuration.",
		//	                  "properties": {
		//	                    "DefaultValue": {
		//	                      "description": "The default value.",
		//	                      "maxLength": 1024,
		//	                      "minLength": 1,
		//	                      "type": "string"
		//	                    },
		//	                    "Key": {
		//	                      "description": "The parameter key name.",
		//	                      "maxLength": 128,
		//	                      "minLength": 1,
		//	                      "pattern": "^[a-zA-Z0-9_-]+$",
		//	                      "type": "string"
		//	                    },
		//	                    "PropertyLocation": {
		//	                      "description": "Specifies where to place the parameter in requests.",
		//	                      "enum": [
		//	                        "HEADER",
		//	                        "BODY",
		//	                        "QUERY_PARAM",
		//	                        "PATH"
		//	                      ],
		//	                      "type": "string"
		//	                    },
		//	                    "Value": {
		//	                      "additionalProperties": false,
		//	                      "description": "Defines how to extract values from HTTP responses.",
		//	                      "properties": {
		//	                        "ContentPath": {
		//	                          "description": "A JSON path expression to extract a value from response body.",
		//	                          "maxLength": 512,
		//	                          "minLength": 1,
		//	                          "pattern": "^\\$(\\.[a-zA-Z0-9_.@\\[\\]\\(\\)-]+)*$",
		//	                          "type": "string"
		//	                        },
		//	                        "HeaderKey": {
		//	                          "description": "The name of an HTTP response header from which to extract the value.",
		//	                          "maxLength": 128,
		//	                          "minLength": 1,
		//	                          "pattern": "^[a-zA-Z0-9_-]+$",
		//	                          "type": "string"
		//	                        }
		//	                      },
		//	                      "type": "object"
		//	                    }
		//	                  },
		//	                  "type": "object"
		//	                },
		//	                "OffsetParameter": {
		//	                  "additionalProperties": false,
		//	                  "description": "Parameter extraction configuration.",
		//	                  "properties": {
		//	                    "DefaultValue": {
		//	                      "description": "The default value.",
		//	                      "maxLength": 1024,
		//	                      "minLength": 1,
		//	                      "type": "string"
		//	                    },
		//	                    "Key": {
		//	                      "description": "The parameter key name.",
		//	                      "maxLength": 128,
		//	                      "minLength": 1,
		//	                      "pattern": "^[a-zA-Z0-9_-]+$",
		//	                      "type": "string"
		//	                    },
		//	                    "PropertyLocation": {
		//	                      "description": "Specifies where to place the parameter in requests.",
		//	                      "enum": [
		//	                        "HEADER",
		//	                        "BODY",
		//	                        "QUERY_PARAM",
		//	                        "PATH"
		//	                      ],
		//	                      "type": "string"
		//	                    },
		//	                    "Value": {
		//	                      "additionalProperties": false,
		//	                      "description": "Defines how to extract values from HTTP responses.",
		//	                      "properties": {
		//	                        "ContentPath": {
		//	                          "description": "A JSON path expression to extract a value from response body.",
		//	                          "maxLength": 512,
		//	                          "minLength": 1,
		//	                          "pattern": "^\\$(\\.[a-zA-Z0-9_.@\\[\\]\\(\\)-]+)*$",
		//	                          "type": "string"
		//	                        },
		//	                        "HeaderKey": {
		//	                          "description": "The name of an HTTP response header from which to extract the value.",
		//	                          "maxLength": 128,
		//	                          "minLength": 1,
		//	                          "pattern": "^[a-zA-Z0-9_-]+$",
		//	                          "type": "string"
		//	                        }
		//	                      },
		//	                      "type": "object"
		//	                    }
		//	                  },
		//	                  "type": "object"
		//	                }
		//	              },
		//	              "required": [
		//	                "OffsetParameter",
		//	                "LimitParameter"
		//	              ],
		//	              "type": "object"
		//	            }
		//	          },
		//	          "type": "object"
		//	        },
		//	        "RequestMethod": {
		//	          "description": "The HTTP method to use.",
		//	          "enum": [
		//	            "GET",
		//	            "POST"
		//	          ],
		//	          "type": "string"
		//	        },
		//	        "RequestParameters": {
		//	          "description": "Request parameters configuration.",
		//	          "items": {
		//	            "additionalProperties": false,
		//	            "description": "Defines a property configuration for connection types.",
		//	            "properties": {
		//	              "AllowedValues": {
		//	                "description": "A list of allowed values for the property.",
		//	                "items": {
		//	                  "type": "string"
		//	                },
		//	                "type": "array"
		//	              },
		//	              "DefaultValue": {
		//	                "description": "The default value for the property.",
		//	                "type": "string"
		//	              },
		//	              "KeyOverride": {
		//	                "description": "A key name to use when sending this property in API requests.",
		//	                "maxLength": 128,
		//	                "minLength": 1,
		//	                "pattern": "^[a-zA-Z0-9_-]+$",
		//	                "type": "string"
		//	              },
		//	              "Name": {
		//	                "description": "The name of the property.",
		//	                "maxLength": 128,
		//	                "minLength": 1,
		//	                "type": "string"
		//	              },
		//	              "PropertyLocation": {
		//	                "description": "Specifies where this property should be included in REST requests.",
		//	                "enum": [
		//	                  "HEADER",
		//	                  "BODY",
		//	                  "QUERY_PARAM",
		//	                  "PATH"
		//	                ],
		//	                "type": "string"
		//	              },
		//	              "PropertyType": {
		//	                "description": "The data type of this property.",
		//	                "enum": [
		//	                  "USER_INPUT",
		//	                  "SECRET",
		//	                  "READ_ONLY",
		//	                  "UNUSED",
		//	                  "SECRET_OR_USER_INPUT"
		//	                ],
		//	                "type": "string"
		//	              },
		//	              "Required": {
		//	                "description": "Indicates whether the property is required.",
		//	                "type": "boolean"
		//	              }
		//	            },
		//	            "required": [
		//	              "Name",
		//	              "Required",
		//	              "PropertyType"
		//	            ],
		//	            "type": "object"
		//	          },
		//	          "type": "array"
		//	        },
		//	        "RequestPath": {
		//	          "description": "The URL path for the REST endpoint.",
		//	          "maxLength": 512,
		//	          "minLength": 1,
		//	          "pattern": "^/[a-zA-Z0-9._~:/?#\\[\\]@!$\u0026'()*+,;={}-]*$",
		//	          "type": "string"
		//	        },
		//	        "ResponseConfiguration": {
		//	          "additionalProperties": false,
		//	          "description": "Configuration for parsing JSON responses from REST API calls.",
		//	          "properties": {
		//	            "ErrorPath": {
		//	              "description": "JSON path expression for error information location.",
		//	              "maxLength": 512,
		//	              "minLength": 1,
		//	              "pattern": "^\\$(\\.[a-zA-Z0-9_.@\\[\\]\\(\\)-]+)*$",
		//	              "type": "string"
		//	            },
		//	            "ResultPath": {
		//	              "description": "JSON path expression for result data location.",
		//	              "maxLength": 512,
		//	              "minLength": 1,
		//	              "pattern": "^\\$(\\.[a-zA-Z0-9_.@\\[\\]\\(\\)-]+)*$",
		//	              "type": "string"
		//	            }
		//	          },
		//	          "required": [
		//	            "ResultPath"
		//	          ],
		//	          "type": "object"
		//	        }
		//	      },
		//	      "type": "object"
		//	    },
		//	    "ValidationEndpointConfiguration": {
		//	      "additionalProperties": false,
		//	      "description": "Configuration for the validation endpoint. Only supports RequestMethod and RequestPath.",
		//	      "properties": {
		//	        "RequestMethod": {
		//	          "description": "The HTTP method to use.",
		//	          "enum": [
		//	            "GET",
		//	            "POST"
		//	          ],
		//	          "type": "string"
		//	        },
		//	        "RequestPath": {
		//	          "description": "The URL path for the REST endpoint.",
		//	          "maxLength": 512,
		//	          "minLength": 1,
		//	          "pattern": "^/[a-zA-Z0-9._~:/?#\\[\\]@!$\u0026'()*+,;={}-]*$",
		//	          "type": "string"
		//	        }
		//	      },
		//	      "type": "object"
		//	    }
		//	  },
		//	  "type": "object"
		//	}
		"rest_configuration": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
			Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
				// Property: EntityConfigurations
				"entity_configurations":   // Pattern: ""
				schema.MapNestedAttribute{ /*START ATTRIBUTE*/
					NestedObject: schema.NestedAttributeObject{ /*START NESTED OBJECT*/
						Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
							// Property: Schema
							"schema":                  // Pattern: ""
							schema.MapNestedAttribute{ /*START ATTRIBUTE*/
								NestedObject: schema.NestedAttributeObject{ /*START NESTED OBJECT*/
									Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
										// Property: FieldDataType
										"field_data_type": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The data type of the field.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: FilterOverrides
										"filter_overrides": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
											Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
												// Property: BetweenConfiguration
												"between_configuration": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
													Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
														// Property: HighBoundKey
														"high_bound_key": schema.StringAttribute{ /*START ATTRIBUTE*/
															Description: "The parameter name used for the upper bound value in a BETWEEN filter operation.",
															Computed:    true,
														}, /*END ATTRIBUTE*/
														// Property: LowBoundKey
														"low_bound_key": schema.StringAttribute{ /*START ATTRIBUTE*/
															Description: "The parameter name used for the lower bound value in a BETWEEN filter operation.",
															Computed:    true,
														}, /*END ATTRIBUTE*/
														// Property: Template
														"template": schema.StringAttribute{ /*START ATTRIBUTE*/
															Description: "A template string for constructing the BETWEEN filter expression.",
															Computed:    true,
														}, /*END ATTRIBUTE*/
													}, /*END SCHEMA*/
													Description: "Configuration that defines how BETWEEN range filter operations are translated into REST API request parameters.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: DateTimeFormat
												"date_time_format": schema.StringAttribute{ /*START ATTRIBUTE*/
													Description: "The date and time format for filter expressions on this field, overriding the global DateTimeFormat.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: FieldName
												"field_name": schema.StringAttribute{ /*START ATTRIBUTE*/
													Description: "An override for the field name to use in filter expressions, if different from the schema field name.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: OperatorMappings
												"operator_mappings": // Pattern: ""
												schema.MapAttribute{ /*START ATTRIBUTE*/
													ElementType: types.StringType,
													Description: "A map of logical filter operators to their field-specific API representations, overriding the global operator mappings.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
											}, /*END SCHEMA*/
											Description: "Configuration that defines per-field overrides for filter behavior, allowing individual fields to customize how filter operations are applied.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: IsNullable
										"is_nullable": schema.BoolAttribute{ /*START ATTRIBUTE*/
											Description: "Indicates whether this field can contain null values.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: IsOrderable
										"is_orderable": schema.BoolAttribute{ /*START ATTRIBUTE*/
											Description: "Indicates whether this field can be used for ordering results.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: IsPartitionable
										"is_partitionable": schema.BoolAttribute{ /*START ATTRIBUTE*/
											Description: "Indicates whether this field can be used for partitioning queries to the data source.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: IsQueryable
										"is_queryable": schema.BoolAttribute{ /*START ATTRIBUTE*/
											Description: "Indicates whether this field can be used in filter predicates when querying data.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Name
										"name": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The name of the field.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: ResponseDateFormat
										"response_date_format": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The format pattern for parsing date values from API responses. Accepts Java DateTimeFormatter patterns, EPOCH_SECONDS, or EPOCH_MILLIS.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
									}, /*END SCHEMA*/
								}, /*END NESTED OBJECT*/
								Description: "The schema definition for this entity.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: SourceConfiguration
							"source_configuration": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
								Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
									// Property: FilterConfiguration
									"filter_configuration": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
										Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
											// Property: BetweenConfiguration
											"between_configuration": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
												Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
													// Property: HighBoundKey
													"high_bound_key": schema.StringAttribute{ /*START ATTRIBUTE*/
														Description: "The parameter name used for the upper bound value in a BETWEEN filter operation.",
														Computed:    true,
													}, /*END ATTRIBUTE*/
													// Property: LowBoundKey
													"low_bound_key": schema.StringAttribute{ /*START ATTRIBUTE*/
														Description: "The parameter name used for the lower bound value in a BETWEEN filter operation.",
														Computed:    true,
													}, /*END ATTRIBUTE*/
													// Property: Template
													"template": schema.StringAttribute{ /*START ATTRIBUTE*/
														Description: "A template string for constructing the BETWEEN filter expression.",
														Computed:    true,
													}, /*END ATTRIBUTE*/
												}, /*END SCHEMA*/
												Description: "Configuration that defines how BETWEEN range filter operations are translated into REST API request parameters.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: DateTimeFormat
											"date_time_format": schema.StringAttribute{ /*START ATTRIBUTE*/
												Description: "The global date and time format for filter expressions.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: FilterMode
											"filter_mode": schema.StringAttribute{ /*START ATTRIBUTE*/
												Description: "The strategy for applying filters to requests.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: FilterStringConfiguration
											"filter_string_configuration": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
												Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
													// Property: QueryParameterName
													"query_parameter_name": schema.StringAttribute{ /*START ATTRIBUTE*/
														Description: "The query parameter name used to send the constructed filter expression string in API requests.",
														Computed:    true,
													}, /*END ATTRIBUTE*/
													// Property: QuoteCharacter
													"quote_character": schema.StringAttribute{ /*START ATTRIBUTE*/
														Description: "The character used to quote values when QuoteStringValues is true. Defaults to double quotes if not specified.",
														Computed:    true,
													}, /*END ATTRIBUTE*/
													// Property: QuoteStringValues
													"quote_string_values": schema.BoolAttribute{ /*START ATTRIBUTE*/
														Description: "Indicates whether string and date values should be wrapped with a quote character in the filter expression.",
														Computed:    true,
													}, /*END ATTRIBUTE*/
												}, /*END SCHEMA*/
												Description: "Configuration for constructing filter expression strings when using the FILTER_STRING filter mode.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: OperatorMappings
											"operator_mappings": // Pattern: ""
											schema.MapAttribute{ /*START ATTRIBUTE*/
												ElementType: types.StringType,
												Description: "A map of logical filter operators to their API-specific string representations.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: StripQuotes
											"strip_quotes": schema.BoolAttribute{ /*START ATTRIBUTE*/
												Description: "Indicates whether surrounding double quotes should be stripped from filter values before processing.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
										}, /*END SCHEMA*/
										Description: "Configuration that defines how filter predicates are applied to REST API requests, supporting both query parameter and filter string strategies.",
										Computed:    true,
									}, /*END ATTRIBUTE*/
									// Property: PaginationConfiguration
									"pagination_configuration": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
										Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
											// Property: CursorConfiguration
											"cursor_configuration": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
												Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
													// Property: LimitParameter
													"limit_parameter": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
														Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
															// Property: DefaultValue
															"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
																Description: "The default value.",
																Computed:    true,
															}, /*END ATTRIBUTE*/
															// Property: Key
															"key": schema.StringAttribute{ /*START ATTRIBUTE*/
																Description: "The parameter key name.",
																Computed:    true,
															}, /*END ATTRIBUTE*/
															// Property: PropertyLocation
															"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
																Description: "Specifies where to place the parameter in requests.",
																Computed:    true,
															}, /*END ATTRIBUTE*/
															// Property: Value
															"value": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
																Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
																	// Property: ContentPath
																	"content_path": schema.StringAttribute{ /*START ATTRIBUTE*/
																		Description: "A JSON path expression to extract a value from response body.",
																		Computed:    true,
																	}, /*END ATTRIBUTE*/
																	// Property: HeaderKey
																	"header_key": schema.StringAttribute{ /*START ATTRIBUTE*/
																		Description: "The name of an HTTP response header from which to extract the value.",
																		Computed:    true,
																	}, /*END ATTRIBUTE*/
																}, /*END SCHEMA*/
																Description: "Defines how to extract values from HTTP responses.",
																Computed:    true,
															}, /*END ATTRIBUTE*/
														}, /*END SCHEMA*/
														Description: "Parameter extraction configuration.",
														Computed:    true,
													}, /*END ATTRIBUTE*/
													// Property: NextPage
													"next_page": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
														Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
															// Property: DefaultValue
															"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
																Description: "The default value.",
																Computed:    true,
															}, /*END ATTRIBUTE*/
															// Property: Key
															"key": schema.StringAttribute{ /*START ATTRIBUTE*/
																Description: "The parameter key name.",
																Computed:    true,
															}, /*END ATTRIBUTE*/
															// Property: PropertyLocation
															"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
																Description: "Specifies where to place the parameter in requests.",
																Computed:    true,
															}, /*END ATTRIBUTE*/
															// Property: Value
															"value": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
																Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
																	// Property: ContentPath
																	"content_path": schema.StringAttribute{ /*START ATTRIBUTE*/
																		Description: "A JSON path expression to extract a value from response body.",
																		Computed:    true,
																	}, /*END ATTRIBUTE*/
																	// Property: HeaderKey
																	"header_key": schema.StringAttribute{ /*START ATTRIBUTE*/
																		Description: "The name of an HTTP response header from which to extract the value.",
																		Computed:    true,
																	}, /*END ATTRIBUTE*/
																}, /*END SCHEMA*/
																Description: "Defines how to extract values from HTTP responses.",
																Computed:    true,
															}, /*END ATTRIBUTE*/
														}, /*END SCHEMA*/
														Description: "Parameter extraction configuration.",
														Computed:    true,
													}, /*END ATTRIBUTE*/
												}, /*END SCHEMA*/
												Description: "Cursor-based pagination configuration.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: OffsetConfiguration
											"offset_configuration": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
												Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
													// Property: LimitParameter
													"limit_parameter": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
														Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
															// Property: DefaultValue
															"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
																Description: "The default value.",
																Computed:    true,
															}, /*END ATTRIBUTE*/
															// Property: Key
															"key": schema.StringAttribute{ /*START ATTRIBUTE*/
																Description: "The parameter key name.",
																Computed:    true,
															}, /*END ATTRIBUTE*/
															// Property: PropertyLocation
															"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
																Description: "Specifies where to place the parameter in requests.",
																Computed:    true,
															}, /*END ATTRIBUTE*/
															// Property: Value
															"value": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
																Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
																	// Property: ContentPath
																	"content_path": schema.StringAttribute{ /*START ATTRIBUTE*/
																		Description: "A JSON path expression to extract a value from response body.",
																		Computed:    true,
																	}, /*END ATTRIBUTE*/
																	// Property: HeaderKey
																	"header_key": schema.StringAttribute{ /*START ATTRIBUTE*/
																		Description: "The name of an HTTP response header from which to extract the value.",
																		Computed:    true,
																	}, /*END ATTRIBUTE*/
																}, /*END SCHEMA*/
																Description: "Defines how to extract values from HTTP responses.",
																Computed:    true,
															}, /*END ATTRIBUTE*/
														}, /*END SCHEMA*/
														Description: "Parameter extraction configuration.",
														Computed:    true,
													}, /*END ATTRIBUTE*/
													// Property: OffsetParameter
													"offset_parameter": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
														Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
															// Property: DefaultValue
															"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
																Description: "The default value.",
																Computed:    true,
															}, /*END ATTRIBUTE*/
															// Property: Key
															"key": schema.StringAttribute{ /*START ATTRIBUTE*/
																Description: "The parameter key name.",
																Computed:    true,
															}, /*END ATTRIBUTE*/
															// Property: PropertyLocation
															"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
																Description: "Specifies where to place the parameter in requests.",
																Computed:    true,
															}, /*END ATTRIBUTE*/
															// Property: Value
															"value": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
																Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
																	// Property: ContentPath
																	"content_path": schema.StringAttribute{ /*START ATTRIBUTE*/
																		Description: "A JSON path expression to extract a value from response body.",
																		Computed:    true,
																	}, /*END ATTRIBUTE*/
																	// Property: HeaderKey
																	"header_key": schema.StringAttribute{ /*START ATTRIBUTE*/
																		Description: "The name of an HTTP response header from which to extract the value.",
																		Computed:    true,
																	}, /*END ATTRIBUTE*/
																}, /*END SCHEMA*/
																Description: "Defines how to extract values from HTTP responses.",
																Computed:    true,
															}, /*END ATTRIBUTE*/
														}, /*END SCHEMA*/
														Description: "Parameter extraction configuration.",
														Computed:    true,
													}, /*END ATTRIBUTE*/
												}, /*END SCHEMA*/
												Description: "Offset-based pagination configuration.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
										}, /*END SCHEMA*/
										Description: "Configuration for handling paginated responses.",
										Computed:    true,
									}, /*END ATTRIBUTE*/
									// Property: RequestMethod
									"request_method": schema.StringAttribute{ /*START ATTRIBUTE*/
										Description: "The HTTP method to use.",
										Computed:    true,
									}, /*END ATTRIBUTE*/
									// Property: RequestParameters
									"request_parameters": schema.ListNestedAttribute{ /*START ATTRIBUTE*/
										NestedObject: schema.NestedAttributeObject{ /*START NESTED OBJECT*/
											Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
												// Property: AllowedValues
												"allowed_values": schema.ListAttribute{ /*START ATTRIBUTE*/
													ElementType: types.StringType,
													Description: "A list of allowed values for the property.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: DefaultValue
												"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
													Description: "The default value for the property.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: KeyOverride
												"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
													Description: "A key name to use when sending this property in API requests.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: Name
												"name": schema.StringAttribute{ /*START ATTRIBUTE*/
													Description: "The name of the property.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: PropertyLocation
												"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
													Description: "Specifies where this property should be included in REST requests.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: PropertyType
												"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
													Description: "The data type of this property.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: Required
												"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
													Description: "Indicates whether the property is required.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
											}, /*END SCHEMA*/
										}, /*END NESTED OBJECT*/
										Description: "Request parameters configuration.",
										Computed:    true,
									}, /*END ATTRIBUTE*/
									// Property: RequestPath
									"request_path": schema.StringAttribute{ /*START ATTRIBUTE*/
										Description: "The URL path for the REST endpoint.",
										Computed:    true,
									}, /*END ATTRIBUTE*/
									// Property: ResponseConfiguration
									"response_configuration": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
										Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
											// Property: ErrorPath
											"error_path": schema.StringAttribute{ /*START ATTRIBUTE*/
												Description: "JSON path expression for error information location.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
											// Property: ResultPath
											"result_path": schema.StringAttribute{ /*START ATTRIBUTE*/
												Description: "JSON path expression for result data location.",
												Computed:    true,
											}, /*END ATTRIBUTE*/
										}, /*END SCHEMA*/
										Description: "Configuration for parsing JSON responses from REST API calls.",
										Computed:    true,
									}, /*END ATTRIBUTE*/
								}, /*END SCHEMA*/
								Description: "Configuration that defines how to make requests to endpoints.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
						}, /*END SCHEMA*/
					}, /*END NESTED OBJECT*/
					Description: "A map of entity configurations.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: GlobalSourceConfiguration
				"global_source_configuration": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
					Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
						// Property: FilterConfiguration
						"filter_configuration": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
							Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
								// Property: BetweenConfiguration
								"between_configuration": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
									Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
										// Property: HighBoundKey
										"high_bound_key": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The parameter name used for the upper bound value in a BETWEEN filter operation.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: LowBoundKey
										"low_bound_key": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The parameter name used for the lower bound value in a BETWEEN filter operation.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: Template
										"template": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "A template string for constructing the BETWEEN filter expression.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
									}, /*END SCHEMA*/
									Description: "Configuration that defines how BETWEEN range filter operations are translated into REST API request parameters.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: DateTimeFormat
								"date_time_format": schema.StringAttribute{ /*START ATTRIBUTE*/
									Description: "The global date and time format for filter expressions.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: FilterMode
								"filter_mode": schema.StringAttribute{ /*START ATTRIBUTE*/
									Description: "The strategy for applying filters to requests.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: FilterStringConfiguration
								"filter_string_configuration": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
									Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
										// Property: QueryParameterName
										"query_parameter_name": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The query parameter name used to send the constructed filter expression string in API requests.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: QuoteCharacter
										"quote_character": schema.StringAttribute{ /*START ATTRIBUTE*/
											Description: "The character used to quote values when QuoteStringValues is true. Defaults to double quotes if not specified.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: QuoteStringValues
										"quote_string_values": schema.BoolAttribute{ /*START ATTRIBUTE*/
											Description: "Indicates whether string and date values should be wrapped with a quote character in the filter expression.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
									}, /*END SCHEMA*/
									Description: "Configuration for constructing filter expression strings when using the FILTER_STRING filter mode.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: OperatorMappings
								"operator_mappings": // Pattern: ""
								schema.MapAttribute{ /*START ATTRIBUTE*/
									ElementType: types.StringType,
									Description: "A map of logical filter operators to their API-specific string representations.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: StripQuotes
								"strip_quotes": schema.BoolAttribute{ /*START ATTRIBUTE*/
									Description: "Indicates whether surrounding double quotes should be stripped from filter values before processing.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
							}, /*END SCHEMA*/
							Description: "Configuration that defines how filter predicates are applied to REST API requests, supporting both query parameter and filter string strategies.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
						// Property: PaginationConfiguration
						"pagination_configuration": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
							Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
								// Property: CursorConfiguration
								"cursor_configuration": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
									Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
										// Property: LimitParameter
										"limit_parameter": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
											Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
												// Property: DefaultValue
												"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
													Description: "The default value.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: Key
												"key": schema.StringAttribute{ /*START ATTRIBUTE*/
													Description: "The parameter key name.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: PropertyLocation
												"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
													Description: "Specifies where to place the parameter in requests.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: Value
												"value": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
													Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
														// Property: ContentPath
														"content_path": schema.StringAttribute{ /*START ATTRIBUTE*/
															Description: "A JSON path expression to extract a value from response body.",
															Computed:    true,
														}, /*END ATTRIBUTE*/
														// Property: HeaderKey
														"header_key": schema.StringAttribute{ /*START ATTRIBUTE*/
															Description: "The name of an HTTP response header from which to extract the value.",
															Computed:    true,
														}, /*END ATTRIBUTE*/
													}, /*END SCHEMA*/
													Description: "Defines how to extract values from HTTP responses.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
											}, /*END SCHEMA*/
											Description: "Parameter extraction configuration.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: NextPage
										"next_page": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
											Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
												// Property: DefaultValue
												"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
													Description: "The default value.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: Key
												"key": schema.StringAttribute{ /*START ATTRIBUTE*/
													Description: "The parameter key name.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: PropertyLocation
												"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
													Description: "Specifies where to place the parameter in requests.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: Value
												"value": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
													Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
														// Property: ContentPath
														"content_path": schema.StringAttribute{ /*START ATTRIBUTE*/
															Description: "A JSON path expression to extract a value from response body.",
															Computed:    true,
														}, /*END ATTRIBUTE*/
														// Property: HeaderKey
														"header_key": schema.StringAttribute{ /*START ATTRIBUTE*/
															Description: "The name of an HTTP response header from which to extract the value.",
															Computed:    true,
														}, /*END ATTRIBUTE*/
													}, /*END SCHEMA*/
													Description: "Defines how to extract values from HTTP responses.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
											}, /*END SCHEMA*/
											Description: "Parameter extraction configuration.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
									}, /*END SCHEMA*/
									Description: "Cursor-based pagination configuration.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: OffsetConfiguration
								"offset_configuration": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
									Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
										// Property: LimitParameter
										"limit_parameter": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
											Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
												// Property: DefaultValue
												"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
													Description: "The default value.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: Key
												"key": schema.StringAttribute{ /*START ATTRIBUTE*/
													Description: "The parameter key name.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: PropertyLocation
												"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
													Description: "Specifies where to place the parameter in requests.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: Value
												"value": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
													Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
														// Property: ContentPath
														"content_path": schema.StringAttribute{ /*START ATTRIBUTE*/
															Description: "A JSON path expression to extract a value from response body.",
															Computed:    true,
														}, /*END ATTRIBUTE*/
														// Property: HeaderKey
														"header_key": schema.StringAttribute{ /*START ATTRIBUTE*/
															Description: "The name of an HTTP response header from which to extract the value.",
															Computed:    true,
														}, /*END ATTRIBUTE*/
													}, /*END SCHEMA*/
													Description: "Defines how to extract values from HTTP responses.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
											}, /*END SCHEMA*/
											Description: "Parameter extraction configuration.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
										// Property: OffsetParameter
										"offset_parameter": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
											Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
												// Property: DefaultValue
												"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
													Description: "The default value.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: Key
												"key": schema.StringAttribute{ /*START ATTRIBUTE*/
													Description: "The parameter key name.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: PropertyLocation
												"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
													Description: "Specifies where to place the parameter in requests.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
												// Property: Value
												"value": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
													Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
														// Property: ContentPath
														"content_path": schema.StringAttribute{ /*START ATTRIBUTE*/
															Description: "A JSON path expression to extract a value from response body.",
															Computed:    true,
														}, /*END ATTRIBUTE*/
														// Property: HeaderKey
														"header_key": schema.StringAttribute{ /*START ATTRIBUTE*/
															Description: "The name of an HTTP response header from which to extract the value.",
															Computed:    true,
														}, /*END ATTRIBUTE*/
													}, /*END SCHEMA*/
													Description: "Defines how to extract values from HTTP responses.",
													Computed:    true,
												}, /*END ATTRIBUTE*/
											}, /*END SCHEMA*/
											Description: "Parameter extraction configuration.",
											Computed:    true,
										}, /*END ATTRIBUTE*/
									}, /*END SCHEMA*/
									Description: "Offset-based pagination configuration.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
							}, /*END SCHEMA*/
							Description: "Configuration for handling paginated responses.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
						// Property: RequestMethod
						"request_method": schema.StringAttribute{ /*START ATTRIBUTE*/
							Description: "The HTTP method to use.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
						// Property: RequestParameters
						"request_parameters": schema.ListNestedAttribute{ /*START ATTRIBUTE*/
							NestedObject: schema.NestedAttributeObject{ /*START NESTED OBJECT*/
								Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
									// Property: AllowedValues
									"allowed_values": schema.ListAttribute{ /*START ATTRIBUTE*/
										ElementType: types.StringType,
										Description: "A list of allowed values for the property.",
										Computed:    true,
									}, /*END ATTRIBUTE*/
									// Property: DefaultValue
									"default_value": schema.StringAttribute{ /*START ATTRIBUTE*/
										Description: "The default value for the property.",
										Computed:    true,
									}, /*END ATTRIBUTE*/
									// Property: KeyOverride
									"key_override": schema.StringAttribute{ /*START ATTRIBUTE*/
										Description: "A key name to use when sending this property in API requests.",
										Computed:    true,
									}, /*END ATTRIBUTE*/
									// Property: Name
									"name": schema.StringAttribute{ /*START ATTRIBUTE*/
										Description: "The name of the property.",
										Computed:    true,
									}, /*END ATTRIBUTE*/
									// Property: PropertyLocation
									"property_location": schema.StringAttribute{ /*START ATTRIBUTE*/
										Description: "Specifies where this property should be included in REST requests.",
										Computed:    true,
									}, /*END ATTRIBUTE*/
									// Property: PropertyType
									"property_type": schema.StringAttribute{ /*START ATTRIBUTE*/
										Description: "The data type of this property.",
										Computed:    true,
									}, /*END ATTRIBUTE*/
									// Property: Required
									"required": schema.BoolAttribute{ /*START ATTRIBUTE*/
										Description: "Indicates whether the property is required.",
										Computed:    true,
									}, /*END ATTRIBUTE*/
								}, /*END SCHEMA*/
							}, /*END NESTED OBJECT*/
							Description: "Request parameters configuration.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
						// Property: RequestPath
						"request_path": schema.StringAttribute{ /*START ATTRIBUTE*/
							Description: "The URL path for the REST endpoint.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
						// Property: ResponseConfiguration
						"response_configuration": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
							Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
								// Property: ErrorPath
								"error_path": schema.StringAttribute{ /*START ATTRIBUTE*/
									Description: "JSON path expression for error information location.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
								// Property: ResultPath
								"result_path": schema.StringAttribute{ /*START ATTRIBUTE*/
									Description: "JSON path expression for result data location.",
									Computed:    true,
								}, /*END ATTRIBUTE*/
							}, /*END SCHEMA*/
							Description: "Configuration for parsing JSON responses from REST API calls.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
					}, /*END SCHEMA*/
					Description: "Configuration that defines how to make requests to endpoints.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: ValidationEndpointConfiguration
				"validation_endpoint_configuration": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
					Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
						// Property: RequestMethod
						"request_method": schema.StringAttribute{ /*START ATTRIBUTE*/
							Description: "The HTTP method to use.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
						// Property: RequestPath
						"request_path": schema.StringAttribute{ /*START ATTRIBUTE*/
							Description: "The URL path for the REST endpoint.",
							Computed:    true,
						}, /*END ATTRIBUTE*/
					}, /*END SCHEMA*/
					Description: "Configuration for the validation endpoint. Only supports RequestMethod and RequestPath.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
			}, /*END SCHEMA*/
			Description: "Configuration for HTTP request and response handling.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: Tags
		// CloudFormation resource type schema:
		//
		//	{
		//	  "description": "Tags to assign to the connection type.",
		//	  "insertionOrder": false,
		//	  "items": {
		//	    "additionalProperties": false,
		//	    "properties": {
		//	      "Key": {
		//	        "maxLength": 128,
		//	        "minLength": 1,
		//	        "type": "string"
		//	      },
		//	      "Value": {
		//	        "maxLength": 256,
		//	        "minLength": 0,
		//	        "type": "string"
		//	      }
		//	    },
		//	    "required": [
		//	      "Key",
		//	      "Value"
		//	    ],
		//	    "type": "object"
		//	  },
		//	  "maxItems": 50,
		//	  "type": "array"
		//	}
		"tags": schema.ListNestedAttribute{ /*START ATTRIBUTE*/
			NestedObject: schema.NestedAttributeObject{ /*START NESTED OBJECT*/
				Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
					// Property: Key
					"key": schema.StringAttribute{ /*START ATTRIBUTE*/
						Computed: true,
					}, /*END ATTRIBUTE*/
					// Property: Value
					"value": schema.StringAttribute{ /*START ATTRIBUTE*/
						Computed: true,
					}, /*END ATTRIBUTE*/
				}, /*END SCHEMA*/
			}, /*END NESTED OBJECT*/
			Description: "Tags to assign to the connection type.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
	} /*END SCHEMA*/

	attributes["id"] = schema.StringAttribute{
		Description: "Uniquely identifies the resource.",
		Required:    true,
	}

	schema := schema.Schema{
		Description: "Data Source schema for AWS::Glue::ConnectionType",
		Attributes:  attributes,
	}

	var opts generic.DataSourceOptions

	opts = opts.WithCloudFormationTypeName("AWS::Glue::ConnectionType").WithTerraformTypeName("awscc_glue_connection_type")
	opts = opts.WithTerraformSchema(schema)
	opts = opts.WithAttributeNameMap(map[string]string{
		"additional_request_parameters":          "AdditionalRequestParameters",
		"allowed_values":                         "AllowedValues",
		"authentication_parameters":              "AuthenticationParameters",
		"authentication_types":                   "AuthenticationTypes",
		"authorization_code":                     "AuthorizationCode",
		"authorization_code_properties":          "AuthorizationCodeProperties",
		"authorization_code_url":                 "AuthorizationCodeUrl",
		"basic_authentication_properties":        "BasicAuthenticationProperties",
		"between_configuration":                  "BetweenConfiguration",
		"client_credentials_properties":          "ClientCredentialsProperties",
		"client_id":                              "ClientId",
		"client_secret":                          "ClientSecret",
		"connection_properties":                  "ConnectionProperties",
		"connection_type":                        "ConnectionType",
		"connection_type_arn":                    "ConnectionTypeArn",
		"connector_authentication_configuration": "ConnectorAuthenticationConfiguration",
		"content_path":                           "ContentPath",
		"content_type":                           "ContentType",
		"cursor_configuration":                   "CursorConfiguration",
		"custom_authentication_properties":       "CustomAuthenticationProperties",
		"date_time_format":                       "DateTimeFormat",
		"default_value":                          "DefaultValue",
		"description":                            "Description",
		"entity_configurations":                  "EntityConfigurations",
		"error_path":                             "ErrorPath",
		"field_data_type":                        "FieldDataType",
		"field_name":                             "FieldName",
		"filter_configuration":                   "FilterConfiguration",
		"filter_mode":                            "FilterMode",
		"filter_overrides":                       "FilterOverrides",
		"filter_string_configuration":            "FilterStringConfiguration",
		"global_source_configuration":            "GlobalSourceConfiguration",
		"header_key":                             "HeaderKey",
		"high_bound_key":                         "HighBoundKey",
		"integration_type":                       "IntegrationType",
		"is_nullable":                            "IsNullable",
		"is_orderable":                           "IsOrderable",
		"is_partitionable":                       "IsPartitionable",
		"is_queryable":                           "IsQueryable",
		"jwt_bearer_properties":                  "JWTBearerProperties",
		"jwt_token":                              "JwtToken",
		"key":                                    "Key",
		"key_override":                           "KeyOverride",
		"limit_parameter":                        "LimitParameter",
		"low_bound_key":                          "LowBoundKey",
		"name":                                   "Name",
		"next_page":                              "NextPage",
		"o_auth_2_grant_type":                    "OAuth2GrantType",
		"o_auth_2_properties":                    "OAuth2Properties",
		"offset_configuration":                   "OffsetConfiguration",
		"offset_parameter":                       "OffsetParameter",
		"operator_mappings":                      "OperatorMappings",
		"pagination_configuration":               "PaginationConfiguration",
		"password":                               "Password",
		"prompt":                                 "Prompt",
		"property_location":                      "PropertyLocation",
		"property_type":                          "PropertyType",
		"query_parameter_name":                   "QueryParameterName",
		"quote_character":                        "QuoteCharacter",
		"quote_string_values":                    "QuoteStringValues",
		"redirect_uri":                           "RedirectUri",
		"request_method":                         "RequestMethod",
		"request_parameters":                     "RequestParameters",
		"request_path":                           "RequestPath",
		"required":                               "Required",
		"response_configuration":                 "ResponseConfiguration",
		"response_date_format":                   "ResponseDateFormat",
		"rest_configuration":                     "RestConfiguration",
		"result_path":                            "ResultPath",
		"schema":                                 "Schema",
		"scope":                                  "Scope",
		"source_configuration":                   "SourceConfiguration",
		"strip_quotes":                           "StripQuotes",
		"tags":                                   "Tags",
		"template":                               "Template",
		"token_url":                              "TokenUrl",
		"token_url_parameters":                   "TokenUrlParameters",
		"url":                                    "Url",
		"username":                               "Username",
		"validation_endpoint_configuration":      "ValidationEndpointConfiguration",
		"value":                                  "Value",
	})

	v, err := generic.NewSingularDataSource(ctx, opts...)

	if err != nil {
		return nil, err
	}

	return v, nil
}
