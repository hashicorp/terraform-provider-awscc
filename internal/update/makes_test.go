...
func TestGetResourceFromLog(t *testing.T) {
	t.Parallel()
	...
	tests := map[string]struct {
		...
		"missing_log_file": {
			setupFile:           false,
			expectError:         true,
			expectedErrorString: "failed to read logs file",
		},
		"empty_log_file": {
			setupFile:        true,
			logContent:       "",
			expectedResource: "", // Empty content returns empty string, not error
			expectError:      false,
		},
		"single_resource": {
			setupFile:        true,
			logContent:       "AWS_S3_Bucket",
			expectedResource: "AWS_S3_Bucket",
			expectError:      false,
		},
		"multiple_resources": {
			setupFile:        true,
			logContent:       "AWS_S3_Bucket\nAWS_EC2_Instance\nAWS_RDS_DBInstance",
			expectedResource: "AWS_RDS_DBInstance", // Should return the last one
			expectError:      false,
		},
	}
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			// Mock file paths
			filePaths := &UpdateFilePaths{
				RunMakesResourceLog: filepath.Join(tempDir, "resource_log.txt"),
			}

			// Setup log file if needed
			if test.setupFile {
				if err := os.WriteFile(filePaths.RunMakesResourceLog, []byte(test.logContent), 0644); err != nil {
					t.Fatalf("failed to create log file: %v", err)
				}
			}

			resource, err := GetResourceFromLog(filePaths)
			...
			if test.expectError {
				...
				if resource != test.expectedResource {
					t.Fatalf("expected resource %q, but got: %q", test.expectedResource, resource)
				}
			}
		})
	}
