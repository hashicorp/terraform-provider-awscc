...
// TestFormatCommitMessage tests the FormatCommitMessage function which generates
// standardized commit messages for CloudFormation schema updates.
// This test verifies that the formatting logic produces expected outputs for various
// update types and scenarios including schema refreshes, new schemas, terraform
// resource generation, documentation updates, changelog entries, and releases.
func TestFormatCommitMessage(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			updateType: "refresh",
			details:    "",
			expected:   "2023-06-26 CloudFormation schemas in us-west-2; Refresh existing schemas.",
		},
		{
			name:       "New type",
			date:       "2023-06-26",
			...
			updateType: "new",
			details:    "",
			expected:   "2023-06-26 CloudFormation schemas in us-east-1; New schemas.",
		},
		{
			name:       "Terraform resources type",
			date:       "2023-06-26",
			region:     "eu-west-1",
			updateType: "terraform-resources",
			details:    "",
			expected:   "2023-06-26 CloudFormation schemas in eu-west-1; Generate Terraform resources.",
		},
		{
			name:       "Docs type",
			date:       "2023-06-26",
			...
			updateType: "docs",
			details:    "",
			expected:   "2023-06-26 Run 'make docs-all'.",
		},
		{
			name:       "Changelog type",
			date:       "2023-06-26",
			...
			details:    "",
			expected:   "Add CHANGELOG entries.",
		},
		{
			name:       "Release type",
			date:       "2023-06-26",
			...
			updateType: "release",
			details:    "1.2.3",
			expected:   "Prepare for v1.2.3 release.",
		},
		{
			name:       "Default type with details",
			date:       "2023-06-26",
			region:     "ap-southeast-1",
			updateType: "default",
			details:    "Custom updates",
			expected:   "2023-06-26 CloudFormation schemas in ap-southeast-1; Custom updates",
		},
		{
			name:       "Default type without details",
			date:       "2023-06-26",
			region:     "ap-southeast-1",
			updateType: "default",
			details:    "",
			expected:   "2023-06-26 CloudFormation schemas in ap-southeast-1; Updates.",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := FormatCommitMessage(tt.date, tt.region, tt.updateType, tt.details)
