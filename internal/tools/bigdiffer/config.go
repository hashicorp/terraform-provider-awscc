...
type config struct {
	...
	changelogPath      string // CHANGELOG.md (item 16 writes its Unreleased FEATURES: block)
	...
	region         string // CloudFormation registry region used for live discovery (-sync)
...
func newConfig(overlayPath string, defaults defaultsBlock, meta metaSchemaBlock) (config, error) {
	...
	return config{
		...
		changelogPath:      filepath.Join(filepath.Dir(outputRoot), "CHANGELOG.md"),
