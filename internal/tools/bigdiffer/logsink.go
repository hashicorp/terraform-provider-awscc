...
// restores the console-only defaults. On failure it warns and leaves the
// console-only defaults in place, so logging never blocks a run.
...
func printActionItems(decisions map[string]policyDecision) {
	...
	sort.Slice(items, func(i, j int) bool { return items[i].cfType < items[j].cfType })
