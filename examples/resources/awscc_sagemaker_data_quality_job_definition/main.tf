...
resource "awscc_s3_bucket" "monitoring" {
  bucket_name = "sagemaker-dataquality-${data.aws_caller_identity.current.account_id}-${data.aws_region.current.region}"
  tags = [{
    key   = "Modified By"
    value = "AWSCC"
  }]
...
resource "awscc_sagemaker_data_quality_job_definition" "example" {
  ...
  data_quality_app_specification = {
    image_uri = "174368400705.dkr.ecr.${data.aws_region.current.region}.amazonaws.com/sagemaker-model-monitor-analyzer:latest"
  }
