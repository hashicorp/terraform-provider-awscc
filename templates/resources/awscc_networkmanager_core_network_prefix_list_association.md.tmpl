...
subcategory: ""
description: |-
  Create a Network Manager Core Network Prefix List Association to associate managed prefix lists with core network segments
---
...
* `core_network_id` - (Required) The ID of the core network to associate the prefix list with.
* `prefix_list_arn` - (Required) The ARN of the managed prefix list to associate.
* `prefix_list_alias` - (Required) An alias for the prefix list (alphanumeric characters only).

## Attribute Reference

In addition to all arguments above, the following attributes are exported:

* `id` - The unique identifier for the prefix list association.

## Import

Network Manager Core Network Prefix List Association can be imported using the `id`, e.g.,

```
$ terraform import awscc_networkmanager_core_network_prefix_list_association.example core-network-12345678|prefix-list-alias
```