...
either gate needs a result distinct from both existing outcomes:
...
**A schema-unchanged failure is not automatically attributable to codegen,
though.** Two distinct situations produce the identical symptom
("schema bytes unchanged, build fails"), and they need different responses:

| Mode | Generated bytes vs. committed | Build result | Cause | Response |
|---|---|---|---|---|
| 1 | **Changed** | new fails | This run's codegen/plan output is different from what's shipping, and the different output is what broke. Attributable to this artifact specifically — the committed file, being what's already shipping unmodified, is still known-good. | `machinery_held`: keep committed file, mark this artifact |
| 2 | **Unchanged** (byte-identical to committed) | fails | Codegen produced the exact same file it always has, and *that same file* — already sitting in the shipped tree — no longer builds. Nothing about generation changed; the only variable left is the build environment (Go version, a stdlib/dependency bump, a shared runtime package). Not attributable to any one artifact — every other schema-unchanged type sharing that environment is equally suspect. | Run-level failure, not a per-artifact overlay edit (see below) |

The distinguishing signal is exactly the generated bytes, which bigdiffer
already has on both sides of the comparison: the freshly regenerated output
(in memory, pre-promotion) and the committed file already on disk. No second
compile pass is needed — diff the two byte-for-byte before deciding how to
route a build failure:

- **Mode 1 (generated bytes differ, build fails): `machinery_held`.** The
  regenerated output is genuinely different and genuinely broken; the
  committed file is untouched and known-good (it's identical to what's
  running in production right now). Proceed as designed below.
- **Mode 2 (generated bytes identical, build fails): not `machinery_held` —
  a run-level environment failure.** Marking one resource "held" here would
  misattribute a systemic break (the environment) as a per-resource codegen
  defect, and — worse — every other schema-unchanged type sharing the same
  broken environment is equally likely to fail the same way, so marking them
  one by one would just be the same non-attributable finding repeated
  hundreds of times. Instead: stop the corpus-wide sweep (or at minimum,
  collapse every Mode 2 occurrence into a single "environment/toolchain
  break, not codegen" report section, not N per-resource entries) and fail
  loudly — this is not something `-update` should promote past or an overlay
  edit should quietly absorb, because keeping "the old file" here doesn't
  actually keep a *verified* file; it keeps a file nobody has re-checked
  against the environment that is about to ship it.

**Proposed: `machinery_held`, a new per-artifact status for Mode 1 only,
orthogonal to `frozen_since` and `suppress_*`.** A schema-unchanged artifact
whose regenerated output differs from the committed file and fails to
generate or compile keeps its **last-good committed file on disk untouched**
(never regress, same invariant §7 already guarantees for schema breaks) and
gets a new overlay marker recording *why the committed output no longer
matches what the current machinery would produce*:
...
- **Only Mode 1 is eligible for this category.** A schema-changed type that
  ...
  the right call), and a Mode 2 failure routes to the run-level path above,
  not to any per-artifact marker.
