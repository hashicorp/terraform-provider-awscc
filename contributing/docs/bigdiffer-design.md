...
- **`-check` enforces it,** in two tiers of a different kind:
    ...
    - **Full, opt-in (`-check-docs-full`):** the registry docs themselves are
      ...
      --providers-schema` (pointed at that extracted schema file), a flag that
      makes the render a pure function of (schema, templates, examples) with
      no further build or Terraform CLI call. This tier is markedly heavier
      than everything else `-check` does — a real whole-provider build plus
      a schema-extraction call — so it stays
