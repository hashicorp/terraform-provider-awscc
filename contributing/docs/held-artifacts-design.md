...
- **`sync`** (AWS, writes): crawl, diff, gate the **whole** corpus.
  ...
  → fail the run. Promote any type whose regenerated output differs from
  committed *and* compiles — valid machinery-driven drift — so an engine change
  self-heals across the corpus at the next `sync` instead of aging silently. A
  hard error promotes nothing (never-regress batch atomicity, unchanged).
...
8. **Doc reconciliation, one pass, last.** This file and `redesign-punchlist.md`
   ...
   (§0 command table, §6, and a §7 note that unchanged-schema failures
   hard-error), the README, and the runbook — together — and delete both
   transient docs.
